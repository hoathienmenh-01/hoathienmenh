...
| GET    | `/character/body-cultivation` | Yes | **Phase 26.0** — trạng thái Luyện Thể. Trả `{ bodyCultivation }` gồm `bodyRealmKey`, `bodyRealmName`, `bodyStage`, `bodyExp`, `bodyExpNext`, `bodyRate`, `bodyCultivating`, `bodyInjuryUntil`, `physiqueKey`, `statBonus { hpMax, power, def, staminaMax, bossDamageReduction }`, `canBreakthrough`, `breakthroughRequirement`, `missingMaterials`. |
| POST   | `/character/body-cultivation/start` | Yes | **Phase 26.0** — bật Luyện Thể (`bodyCultivating=true`) riêng với Nhập Định. Body optional `{ bodyCultivating?: boolean }` chỉ validate input, server set true. Trả `{ bodyCultivation }`; lỗi `NO_CHARACTER`, `UNAUTHENTICATED`, `INVALID_INPUT`. |
| POST   | `/character/body-cultivation/stop` | Yes | **Phase 26.0** — tắt Luyện Thể (`bodyCultivating=false`) riêng với Nhập Định. Body optional như start. Trả `{ bodyCultivation }`. |
| POST   | `/character/body-cultivation/breakthrough` | Yes | **Phase 26.0** — đột phá đại cảnh Luyện Thể thủ công. No body. Server kiểm tra peak body stage, đủ `bodyExp`, không vượt `bodyRealmOrder > qiRealmOrder + 1`, đủ material/pill; consume item trong transaction, ghi `BodyBreakthroughAttemptLog`, success lên realm kế tiếp/stage 1, fail trừ một phần bodyExp và set `bodyInjuryUntil`. Trả `{ bodyBreakthrough: { success, successRate, status } }`. Errors: `NO_CHARACTER`, `NOT_AT_PEAK`, `MAX_REALM`, `QI_GATE`, `INSUFFICIENT_EXP`, `MISSING_MATERIALS`, `MISSING_PILL`. Không grant linhThach/tienNgoc. |
...
| GET    | `/character/achievements` | Yes | **Phase 11.10 + 46.0** — state thành tựu server-authoritative: `{ achievements[] }` merge catalog visible + progress `{ achievementKey, progress, completedAt, claimedAt, def }`. Hidden achievement chỉ hiện khi đã có row/progress. |
| POST   | `/character/achievement/claim` | Yes | **Phase 11.10 + 46.0** — Body `{ achievementKey }`. Atomic claim 1 lần bằng CAS `claimedAt=null`; grant reward nhỏ qua `CurrencyLedger` / `ItemLedger`, unlock title cosmetic nếu catalog link. Errors: `ACHIEVEMENT_NOT_FOUND`, `ACHIEVEMENT_NOT_COMPLETED`, `ACHIEVEMENT_ALREADY_CLAIMED`, `NO_CHARACTER`. |
| GET    | `/character/reputation/me` | Yes | **Phase 46.0** — Uy danh theo 8 nhóm (`TIEN_DAO`, `DAN_DAO`, `CHIEN_DAU`, `BI_CANH`, `TONG_MON`, `XA_HOI`, `SU_KIEN`, `THUONG_HOI`): `{ reputation[] }` với `{ group, score, dailyGain, dailyCap, lastGainedAt, def }`. Read-only, không grant. |
| GET    | `/character/long-term-goals/me` | Yes | **Phase 46.0** — Mục tiêu dài hạn foundation: `{ goals[] }` với `{ goalKey, progress, completedAt, def }` cho realm/body/pet/dungeon/boss/sect. Read-only, UI progress panel. |
...
| GET    | `/character/phap-bao/list`             | Yes | **Phase 23.5/23.7** — danh sách pháp bảo character đang sở hữu + catalog metadata. KHÔNG mutate. Server verify auth + character, filter `InventoryItem` theo `PHAP_BAO_CATALOG`, map → `PhapBaoView { inventoryItemId, def, equippedSlot, refineLevel, starLevel, awakenStage, canEquip, requiredRealmOrder, powerScore }`, `canEquip = canEquipPhapBao(character.realmOrder, def)`, trả `{ ok: true, data: { items, catalog } }`. Errors: 401 `UNAUTHORIZED`, 404 `NO_CHARACTER`. **Pháp bảo là slot riêng** (`ARTIFACT_1/2/3`), không tính vào 8 trang bị chính. |
| GET    | `/character/phap-bao/:inventoryItemId/preview` | Yes | **Phase 23.5/23.7** — read-only preview passive bonus + active skill + refine/star/awaken cost kế tiếp. Param `inventoryItemId` (path, string ≤ 64). Server verifies ownership, lookup catalog, computes `passiveBonus`, `activeSkill`, `powerScore`, `refineCost`, `starCost`, `awakenCost`; null cost means max cap or missing prerequisites. Outcome `{ ok: true, data: { preview: PhapBaoPreview { ..., starUpEnabled: true, awakenEnabled: true } } }`. Errors: 401 `UNAUTHORIZED`, 404 `NO_CHARACTER`/`INVENTORY_ITEM_NOT_FOUND`/`PHAP_BAO_NOT_FOUND`. |
| POST   | `/character/phap-bao/:inventoryItemId/star-up` | Yes | **Phase 23.7** — persisted Pháp Bảo star-up. No body; path id identifies the owned artifact. Server checks ownership, `locked=false`, `realmOrder >= requiredRealmOrder`, `starLevel < min(starCap, 5)`, materials + linh thạch; consumes via `ItemLedger` + `CurrencyLedger`, then guarded `updateMany` increments `phapBaoStarLevel`. Returns `{ ok: true, data: { phapBao: PhapBaoUpgradeResult { item, cost, consumedMaterials, nextPreview } } }`. Errors include `REALM_TOO_LOW`, `MAX_STAR_REACHED`, `INSUFFICIENT_MATERIAL`, `INSUFFICIENT_FUNDS`, `CONCURRENT_UPGRADE`. |
| POST   | `/character/phap-bao/:inventoryItemId/awaken` | Yes | **Phase 23.7** — persisted Pháp Bảo awaken. No body. Server checks ownership, lock, realm, quality TIEN/THAN, tier/awaken cap, `starLevel >= 3 + currentAwakenStage`, `refineLevel >= min(refineCap, 2 + currentAwakenStage×2)`, materials + linh thạch; consumes ledger and guarded `updateMany` increments `phapBaoAwakenStage`. Errors include `QUALITY_TOO_LOW`, `STAR_TOO_LOW`, `REFINE_TOO_LOW`, `MAX_AWAKEN_REACHED`, `CONCURRENT_UPGRADE`. |
| POST   | `/character/phap-bao/:inventoryItemId/refine` | Yes | **Phase 23.7** — persisted Pháp Bảo refine using dedicated artifact cost curve (higher than normal equipment). No body. Server checks ownership, lock, `refineLevel < refineCap`, cost/material/protection if required; consumes ledger and guarded `updateMany` increments `refineLevel`. Errors include `MAX_REFINE_REACHED`, `INSUFFICIENT_MATERIAL`, `INSUFFICIENT_FUNDS`, `CONCURRENT_UPGRADE`. |

Tick EXP thực hiện bởi BullMQ processor `cultivation.processor.ts`. WS event `cultivate:tick` emit per-user khi tick xong. **Phase 26.0** adds `body-cultivate:tick` payload `{ characterId, expGained, bodyExp, bodyExpNext, bodyRealmKey, bodyStage, stagedUp }`; `CharacterStatePayload` also includes body cultivation fields (`bodyRealmKey/bodyRealmName/bodyStage/bodyExp/bodyExpNext/bodyRate/bodyCultivating/bodyInjuryUntil/physiqueKey/bodyStatBonus`) in `state:update`. `CharacterStatePayload.title: string \| null` (Phase 11.9.C) push qua `state:update` event để FE hiển thị title đang trang bị trong topbar.
...
| GET    | `/admin/achievement-reputation/catalog` | ADMIN/MOD | **Phase 46.0** — read-only catalog summary `{ achievements, titles, reputationGroups, longTermGoals }` cho admin review. |
| GET    | `/admin/users/:id/achievement-reputation` | ADMIN/MOD | **Phase 46.0** — read-only progress snapshot theo userId/email/id: `{ userId, characterId, characterName, achievements[], titles[], reputation[], longTermGoals[] }`. 404 nếu user/character không tồn tại. |
