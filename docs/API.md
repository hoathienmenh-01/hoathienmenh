...
| GET    | `/character/phap-bao/list`             | Yes | **Phase 23.5** — danh sách pháp bảo character đang sở hữu + catalog metadata. KHÔNG mutate. Server (1) verify auth + character; (2) load `InventoryItem` của character, filter theo `PHAP_BAO_CATALOG`; (3) map mỗi row → `PhapBaoView { inventoryItemId, def: PhapBaoDefView, equippedSlot, refineLevel, starLevel (foundation=0), awakenStage (foundation=0), canEquip, requiredRealmOrder, powerScore }`; (4) `canEquip = canEquipPhapBao(character.realmOrder, def)`; (5) trả `{ ok: true, data: { items: PhapBaoView[], catalog: PhapBaoDefView[] } }` — `catalog` cho FE hiển thị pháp bảo chưa sở hữu (locked entry). Errors: 401 `UNAUTHORIZED`, 404 `NO_CHARACTER`. **Pháp bảo là slot riêng** (`ARTIFACT_1/2/3`), không tính vào 8 trang bị chính. |
| GET    | `/character/phap-bao/:inventoryItemId/preview` | Yes | **Phase 23.5** — read-only preview passive bonus + active skill + cost cường hóa kế tiếp cho 1 pháp bảo. Param `inventoryItemId` (path, string ≤ 64). Server (1) verify ownership (`InventoryItem.characterId === currentCharacterId`); (2) lookup `getPhapBaoByKey(item.itemKey)` → 404 `PHAP_BAO_NOT_FOUND` nếu không thuộc catalog; (3) compute `passiveBonus = computePhapBaoPassiveBonus({ def, starLevel, refineLevel, awakenStage })` (clamp 50% power-budget cap); (4) `activeSkill = computePhapBaoActiveSkillPreview({ def, starLevel })` (unlock khi `starLevel >= def.activeSkill.unlockStar`); (5) `refineCost = safeRefineCost(def, refineLevel)` / `starCost = safeStarCost(def, starLevel)` / `awakenCost = safeAwakenCost(def, starLevel, awakenStage)` — null nếu đã max hoặc condition (quality TIEN+ tier ≥ 5 + starLevel ≥ 1) chưa đạt. Outcome `{ ok: true, data: { preview: PhapBaoPreview { ..., passiveBonus, activeSkill, refineCost, starCost, awakenCost, starUpEnabled: false, awakenEnabled: false } } }`. Errors: 401 `UNAUTHORIZED`, 404 `NO_CHARACTER`/`INVENTORY_ITEM_NOT_FOUND`/`PHAP_BAO_NOT_FOUND`. **Foundation**: `starUpEnabled` + `awakenEnabled` luôn `false` (mutate endpoint defer Phase 23.6 / 25.1). Refine pháp bảo dùng nguyên `POST /character/refine` (reuse `InventoryItem.refineLevel`). Equip dùng `POST /inventory/equip` (realm gate đã có từ Phase 23.2). |
