...
## Phase 26.1 — Alchemy V2 balance invariants

- Đan Sư có 9 cấp; craft gate là `recipeTier <= alchemyLevel`, `furnaceLevel >= recipe.furnaceLevel`, realm đủ, đủ nguyên liệu và linh thạch. `alchemyLevel` chỉ tăng bằng alchemy EXP từ craft, không từ topup.
- Success rate = base + lower-tier bonus (+5% mỗi cấp Đan Sư vượt recipe, cap +20%) + furnace bonus (cap +8%) + mastery bonus nhỏ (cap +5%), rồi clamp `[5%, 98%]`.
- Craft fail vẫn consume nguyên liệu/linh thạch và nhận 20% `alchemyExpReward`; success nhận full reward nhân theo phẩm đan.
- Grade base distribution khi success: Hạ 45%, Trung 32%, Thượng 16%, Cực 6%, Đan Vân 1%. Bonus mastery/furnace có thể dịch lên phẩm cao nhưng Đan Vân cap 5%, Cực phẩm cap 18%, và `maxOutputGrade` không vượt recipe.
- Grade multiplier: Hạ 0.85, Trung 1.0, Thượng 1.15, Cực 1.3, Đan Vân 1.5. Effect clamp theo recipeTier để tier thấp phẩm cao không vượt tier cao trung phẩm.
