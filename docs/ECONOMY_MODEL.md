...
| Sect Season Champion reward (Phase 15.7) | `sect-season-reward.service.ts` `grantChampionForSeason` (cron + admin manual) | `MAIL_CLAIM` (khi member claim mail) | `SectSeasonRewardGrant` UNIQUE `(seasonKey, 'CHAMPION', characterId)` — cap 100 member/sect/season theo characterId ASC; cron retry KHÔNG double |
