...
### Testing
- [x] **api 1133** + **web 2605** + **shared 950** = **4688+ vitest** (web baseline 2605 as of 2026-05-19; shared 950; api 1133+). Stale snapshot cũ "557 test" của session 9d đã bị thay; mỗi PR mới phải verify số này không giảm.
...
- [x] **M7 — CSP production review**: Helmet 11 directive prod CSP (`apps/api/src/main.ts`). Env-gated qua `SENTRY_ENABLED`.
...
- [x] **Refresh token revoke chain logging**: đã có reuse-detection + admin endpoint `GET /admin/users/:id/sessions` xem chain (Phase 18.2, PR #697).
...
### QA + Launch
- [ ] **Runtime smoke tích hợp**: execute `docs/QA_CHECKLIST.md` 13 section trên local hoặc staging (15 phút). Default suite: 37 scripts (`pnpm smoke:all`).
...
- [x] M7 CSP review — Helmet 11 directive prod CSP (`apps/api/src/main.ts`)
- [x] Sentry + structured logs — `apps/api/src/observability/sentry.ts` + `@xuantoi/logger/backend` + `NestLoggerAdapter`
- [x] Backup DB daily script — `scripts/restore-drill.mjs` (automated backup/restore drill)
...
**Có thể defer đến sau beta** (nhiều items đã DONE):
- [x] M10 Shop daily limit — `shop.service.ts` pre-check + `dailyLimit` per item
- PvP cốc đấu (Phase 9, ngoài scope beta)
- [x] Reforge/enchant — `refine.service.ts` (15 levels) + `equipment.service.ts` (reforge + enchant)
- [x] Realm-based skills — 25 skills across 4 tiers × 5 elements
- [x] Buff/debuff — `buff.service.ts` + wired into cultivation/combat
- [x] Higher-tier dungeons — 12 dungeons cover luyen_khi→hoa_than
- [x] Achievement system — `achievement.service.ts` + wired into controllers

---

_Last updated: 2026-05-19 — Beta Polish Pack PR (tasks #7-#10)_  
