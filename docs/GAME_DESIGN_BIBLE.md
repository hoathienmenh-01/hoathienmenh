...
|---|---|---|
| **Trang bị** | 9 slot, 5 quality, +bonuses. **Phase 23.2** thêm progression metadata derive cho equipment: 28 cảnh giới → 10 tầng trang bị, grade I/II/III trong tier, `requiredRealmOrder` server-authoritative khi equip, `powerBudget/computedPowerScore`, cap cường hóa/socket theo tier+quality. Quality/phẩm cấp đại diện cho sức mạnh và độ hiếm trong cùng equipmentTier: PHAM < LINH < HUYEN < TIEN < THAN. **Phase 23.3** layer Set Bonus 2/4/6 + 5-kind Gear Resonance capped tổng ≤30%. **Phase 23.4** chốt economy/resource sink: ghép phẩm, phân giải, cường hóa/khảm/tháo gem/reforge có cost curve + cap; bảo hộ phù là monetization-safe hook cho Phase 25.1. **Phase 23.5** mở hệ **Pháp Bảo / Advanced Artifact**: slot riêng (`ARTIFACT_1/2/3`) ngoài 8 trang bị chính, catalog foundation 10 pháp bảo với `artifactTier 1–10`, `requiredRealmOrder`, passive capped 50% power-budget, active cooldown floor 30s. **Phase 23.7** bật progression thật: persisted star 1–5, refine 0..cap, awaken 0–3; server-authoritative cost/ledger/anti-duplicate; UI confirm thăng sao/thức tỉnh/luyện khí. Pháp bảo là **build identity**, không phải auto-win — contribution mục tiêu 5–20% theo giai đoạn. | Phase 25.1 wire `refine_protection_charm` / shard / material acceleration vào Battle Pass / Monthly Card theo nguyên tắc premium-safe: không bán pháp bảo top trực tiếp, không bán max-star/max-awaken, không vượt `requiredRealmOrder`; Phase 24.2/24.3 final QA/polish. |
