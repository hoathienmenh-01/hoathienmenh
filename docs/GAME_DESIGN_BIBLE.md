...
|---|---|---|
| **Trang bị** | 9 slot, 5 quality, +bonuses. **Phase 23.2** thêm progression metadata derive cho equipment: 28 cảnh giới → 10 tầng trang bị, grade I/II/III trong tier, `requiredRealmOrder` server-authoritative khi equip, `powerBudget/computedPowerScore`, cap cường hóa/socket theo tier+quality. Quality/phẩm cấp đại diện cho sức mạnh và độ hiếm trong cùng equipmentTier: PHAM < LINH < HUYEN < TIEN < THAN, nhân trực tiếp vào powerBudget/computedPowerScore/stat budget; visual chỉ đọc quality thật để đổi màu/viền/glow. **Phase 23.3** layer Set Bonus 2/4/6 + 5-kind Gear Resonance (same-tier/enhance/quality/elemental/hybrid) capped tổng ≤30%. **Phase 23.4** chốt economy/resource sink: ghép phẩm 3 món cùng tier/slot/quality/family → 1 món quality cao hơn (PHAM→LINH→…→THAN, THAN không ghép tiếp, không ghép khác tier hoặc lên tier cao), phân giải đồ cũ lấy material + linh thạch (yield strictly < merge cost — anti-infinite-sink), cường hóa/khảm/tháo gem/reforge có cost curve + cap đồng nhất, bảo hộ phù là monetization-safe hook cho Phase 25.1. **Phase 23.5** mở hệ **Pháp Bảo / Advanced Artifact**: slot riêng (`ARTIFACT_1/2/3`) ngoài 8 trang bị chính, catalog foundation 10 pháp bảo (`packages/shared/src/phap-bao.ts`) với `artifactTier 1–10` map 28 cảnh giới + `requiredRealmOrder` server-gated, passive bonus capped 50% power-budget, active skill có cooldown floor 30s. Foundation chỉ bật luyện khí (reuse `/character/refine`); thăng sao + thức tỉnh defer Phase 23.6/25.1. Pháp bảo là **build identity**, không phải auto-win — contribution 10–18% tổng power. | Phase 25.1 wire `refine_protection_charm` vào Battle Pass / Monthly Card + Pháp Bảo premium-safe hook (mảnh / nguyên liệu luyện khí); Phase 23.6 enable star-up + awaken persistence + visual rarity FX; gem/refine/reforge/enchant/Ngũ Hành + pháp bảo vẫn là build depth trong tier, không thay thế cảnh giới. |
...
| **Đan dược** | `PILL_HP/MP/EXP` consume one-shot. Phase 11.X.A catalog luyện đan đã có (`packages/shared/src/alchemy.ts`) — 13 recipe baseline cover toàn bộ pill HP/MP/EXP × PHAM..THAN; helpers `simulateAlchemyAttempt/simulateAlchemyBulk` deterministic | Phase 11.X.B: module `apps/api/src/modules/alchemy/` + `Character.alchemyFurnaceLevel` + `AlchemyAttempt` Prisma model + service `attemptAlchemy` qua ItemLedger atomic + idempotency qua attemptId |
...
| **Tài năng / Thần Thông** | Phase 11.7.A catalog đã có (`packages/shared/src/talents.ts`) — 14 talent baseline = 7 passive (atk/hp/regen/def/drop/exp/anti-counter) + 7 active "thần thông" (AOE damage / single CC / heal / DOT / true damage / utility) cover 5 element + neutral; helpers `talentsAvailableAtRealm/computeTalentPointBudget (mỗi 3 realm = 1 point) /canCharacterLearnTalent/composePassiveTalentMods/simulateActiveTalent` deterministic | Phase 11.7.B: Prisma `CharacterTalent { id, characterId, talentKey, learnedAt, mpCooldownUntil }` UNIQUE `(characterId, talentKey)` + service `learnTalent (validate qua canCharacterLearnTalent + persist + giảm point budget)` + `useActiveTalent (check cd + mp + simulateActiveTalent + apply combat hook + atomic mp consume)` + wire `composePassiveTalentMods` vào `CharacterStatService.computeStats` last layer + REST `GET /talent/available, POST /talent/learn, POST /talent/use, GET /talent/points` |
