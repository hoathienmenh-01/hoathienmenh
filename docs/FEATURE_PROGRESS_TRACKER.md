...
| 44 | Phase 45.1 — Feature Flags Wire + Remote Config History | DONE | 12 feature flags defined in catalog but not wired into module controllers/services. VISUAL_EFFECTS flag only in catalog, not FE runtime. | Wire 11 flags into BE modules. Create `useVisualEffects` composable for FE. | Multiple controller/service files, `useVisualEffects.ts` | 11 flags wired in BE; VISUAL_EFFECTS composable for FE; 30/30 flags now wired. PR #689. | 2026-05-28 |
| 45 | Phase 18.2 — Suspicious Login Detection + WS Notify | DONE | Session infrastructure already complete. Missing: suspicious login detection when concurrent sessions from different IP within 5 min. | Add `detectSuspiciousLogin` to SessionService. Wire into AuthService.issueTokens. Add `security:alert` WS event type. Add 4 tests. | `session.service.ts`, `auth.service.ts`, `auth.module.ts`, `ws-events.ts` | Suspicious login detection works. 4 tests pass. PR #690. | 2026-05-28 |
