...
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), Phase 12 Story PR-1→PR-6 + Foundation Extension + Late-game wire all **CLOSED** ✅ (#425–#433), **Phase 12.2.B/C/12.3** **CLOSED** ✅ (#434–#436), **E2E spec #22 + listForUser fallback** **CLOSED** ✅ (#437), **Phase 12.4 per-monster MonsterDef.lootTable polish** **CLOSED** ✅ (#438), **Story Foundation Late-game encounter wire** **CLOSED** ✅ (#439), **Phase 12 Story discoverability — QuestView dungeon hint cho kill+monster step** **CLOSED** ✅ (#440), **docs handoff sync post #440** **CLOSED** ✅ (#441), **Phase 12.5 dungeon balance tuning** **CLOSED** ✅ (#442), **Concurrency phase 2** **CLOSED** ✅ (#422 + #443–#447), **Phase 12.6 Boss-by-region auto-spawn — partial unique per (status, regionKey) + heartbeat loop per region + multi-region UI tabs** **CLOSED** ✅ (this PR).
- **In-flight**: this PR (Phase 12.6 — Boss-by-region auto-spawn: migration `20260523000000_phase_12_6_world_boss_region_key` adds `regionKey TEXT NOT NULL DEFAULT 'world'` + drops old `WorldBoss_status_active_unique` + creates `WorldBoss_status_region_active_unique` partial unique on `(status, regionKey) WHERE status='ACTIVE'`. BossService heartbeat loops `bossSpawnRegions()` from shared catalog and spawns up to 1 boss per region; admin spawn supports explicit `regionKey` + auto-derives from `bossKey` def for backwards-compat; new `listActive()` + `getCurrentByRegion()` endpoints + multi-region FE tabs).
- **Test baseline (post this PR)**: api **2003** (+8 vs #447 baseline 1995: 8 region-isolation tests in `boss.service.region.test.ts` covering cross-region spawn, heartbeat fan-out, region skip, admin region isolation, mismatch validation, listActive sort, getCurrentByRegion filter, schema default `'world'`. Concurrency suite rewritten for per-region invariants kept at +4 tests). shared **1376** (+6 from #447 baseline 1370: `bossSpawnRegions`, `bossesByRegion`, `WORLD_BOSS_REGION_KEY` cross-checks). web **1163**. total **4542 vitest**. E2E golden-path **22 spec**. Smoke scripts **28 module**.
...
|---|---|---|
| `apps/api` | **2003** | +8 từ Phase 12.6 region-isolation suite `boss.service.region.test.ts` (cross-region race, heartbeat fan-out, region skip, admin region isolation, def↔region mismatch validation, listActive sort, getCurrentByRegion filter, schema default `'world'` backfill). Concurrency suite rewritten cho per-region invariant nhưng giữ +4 test count. Carryover #447 partial unique + #446 chat failover. |
| `packages/shared` | **1376** | +6 từ Phase 12.6 (`bossSpawnRegions()` derives union catalog regions ∪ `'world'` for legacy; `bossesByRegion(regionKey)` returns defs filtered + `WORLD_BOSS_REGION_KEY` constant). Carryover #442 Phase 12.5 catalog tuning. |
| `apps/web` | **1163** | BossView refactor cho multi-region tabs giữ count cũ — 12 BossView spec migrate tới `getActiveBosses` mock pattern. |
| **Total** | **4542 vitest** | All green local. **GitHub CI runners**: verify khi PR push. |
...
5. **Achievement / Title / Buff systems integration** (Large PR candidate, Phase 13 prep) — 3 hệ thống live-but-isolated (achievement registers events nhưng UI minimal; title catalog có nhưng chưa wire vào character header; buff infrastructure ở combat nhưng chưa có UI status bar). Gộp 1 PR vì shared UI/i18n/event hook layer.
