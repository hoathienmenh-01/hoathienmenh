...
- **Current `main` commit**: post PR #430 merged (`feat(web): Phase 12 Story Runtime MVP — QuestView.vue list + filter + accept/claim UI`) + PR #429 (Phase 12 Story Foundation Extension Kim Đan + Nguyên Anh catalog) + PR #428 (Phase 12 PR-4 NPC dialogue UI) + PR #427 (Phase 12 PR-3 Quest claim) + PR #426 (Phase 12 PR-2 Quest runtime persistence) + PR #425 (Phase 12 PR-1 catalog foundation). Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), **Phase 12 Story PR-1** catalog **CLOSED** ✅ (#425), **Phase 12 Story PR-2** Quest runtime persistence **CLOSED** ✅ (#426), **Phase 12 Story PR-3** Quest claim/reward idempotency **CLOSED** ✅ (#427), **Phase 12 Story PR-4** NPC dialogue UI **CLOSED** ✅ (#428), **Phase 12 Story Foundation Extension** Kim Đan + Nguyên Anh catalog **CLOSED** ✅ (#429), **Phase 12 Story Runtime MVP** Quest UI **CLOSED** ✅ (#430). **Phase 12 Story PR-5 Main storyline Chapter 1 playable** (`phamnhan_main_01` accept → progress → claim end-to-end + admin quest-track harness + E2E golden-path) **OPEN** via this PR.
- **In-flight**: this PR = Phase 12 Story PR-5 Main storyline Chapter 1 playable. **API**: `POST /admin/users/:id/quest-track` admin seed harness wrap `QuestService.track()` cho kind kill/collect (CANNOT seed talk/explore/choice — gameplay-driven only) + 13 unit test (`admin-grant-quest-track.service.test.ts` covering positive, partial, validation, RBAC, audit). **E2E**: `golden.spec.ts` §21 phamnhan_main_01 end-to-end (onboard → /quests AVAILABLE → click accept → progress talk×2 via API → admin track kill 3 son_thu → reload COMPLETED → click claim → CLAIMED + linhThach +100 + so_kiem item +1). **No new player-facing endpoint**, **no Prisma migration**, **no FE state mutation**.
- **Test baseline (post Phase 12 Story PR-5)**: api **1920** (+13 from `admin-grant-quest-track.service.test.ts`), shared **1328**, web **1126**, total **4374 vitest**. E2E golden-path **21/21 spec** (+1 §21 phamnhan_main_01 playable). Smoke scripts **27 module**. CI expected GREEN.
- **Top priority next session**: (1) **Phase 12 Story PR-6 Combat hook → quest track auto** — wire `CombatService` end-of-encounter callback gọi `QuestService.track('kill', 'monster', monsterKey, n)` cho mọi kill production-flow (vs admin seed PR-5 chỉ E2E). Includes integration test combat → quest progress + ledger row + cooldown idempotency. Medium full-stack; (2) Phase 12.2.B DungeonTemplate + DungeonRun runtime (Medium-Large, **Prisma**); (3) smoke:daily-login multi-day positive (Small); (4) Concurrency tests phase 2 backlog. Detail §6.
...
| PR | Title | Type |
|---|---|---|
| **this PR** | `feat(api,web): Phase 12 Story PR-5 — Main storyline Chapter 1 playable (phamnhan_main_01 end-to-end accept → progress → claim, admin quest-track endpoint + E2E golden-path)` | medium full-stack feature + tests, **no Prisma migration**, **no new player endpoint** |
