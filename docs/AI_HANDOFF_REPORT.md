...
- **Current `main` commit**: post PR #437 merged (`fix(api,web): DungeonRun listForUser activeRun fallback COMPLETED + E2E spec #22 dungeon-run flow`) + PR #436 (Phase 12.3 DungeonRun per-encounter loot wire) + PR #435 (Phase 12.2.C DungeonRun FE UI) + PR #434 (Phase 12.2.B DungeonTemplate + DungeonRun runtime) + PR #433 (Phase 12 Story Foundation Late-game wire) + PR #432 (Phase 12 Story PR-6 Combat kill hook → quest track) + PR #431 (Phase 12 Story PR-5 Chapter 1 playable) + PR #430 (Phase 12 Story Runtime MVP QuestView UI) + PR #429 (Foundation Extension Kim Đan + Nguyên Anh catalog) + PR #428 (PR-4 NPC dialogue UI). Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), Phase 12 Story PR-1→PR-6 + Foundation Extension + Late-game wire all **CLOSED** ✅ (#425–#433), **Phase 12.2.B DungeonTemplate + DungeonRun runtime** **CLOSED** ✅ (#434), **Phase 12.2.C DungeonRun FE UI** **CLOSED** ✅ (#435), **Phase 12.3 DungeonRun per-encounter loot wire** **CLOSED** ✅ (#436), **E2E spec #22 dungeon-run flow + listForUser fallback** **CLOSED** ✅ (#437). **Phase 12.4 per-monster MonsterDef.lootTable polish** (this PR) **OPEN**.
- **In-flight**: this PR = Phase 12.4 per-monster `MonsterDef.lootTable` polish (Medium full-stack). **Context**: Phase 12.3 (#436) đã wire `rollDungeonLoot(dungeon.key, 2)` per-encounter cho `DungeonRunService.nextEncounter` + `CombatService` WON path. Hạn chế: drop pool single-tier per dungeon → boss/elite cùng dungeon drop giống beast routine. Phase 12.4 mở `MonsterDef.lootTable?: readonly LootEntry[]` (optional override) + `rollMonsterLoot(monsterKey, count)` helper. Resolve order: nếu `monster.lootTable` defined + non-empty → roll từ override; else fallback `DUNGEON_LOOT[dungeon.key]` (legacy path). **Schema**: Move `LootEntry`/`RolledLoot` types từ `items.ts` qua `combat.ts` (avoid circular import — `MonsterDef` cần `LootEntry`, items.ts đã import `ElementKey` từ combat). Re-export từ items.ts cho backward compat. **Seed override**: 5 monster boss/elite (1 ELITE `kim_dieu_thuong_phong` + 4 BOSS `thuy_thanh_long_vuong` / `chu_tuoc_huyet_dieu` / `tho_dia_lao_tu` / `cuu_la_huyen_quan`) — bias toward themed equipment + skill_book pity weight 5-6 (vs dungeon 2-3) + `linh_can_dan` pity 3 trên endgame boss. **Wire**: `DungeonRunService.nextEncounter` + cả 2 call site `CombatService` (skill flow + talent flow) — `monster.lootTable.length > 0 ? rollMonsterLoot : rollDungeonLoot`. Server-authoritative — FE KHÔNG self-grant.
- **Test baseline (post Phase 12.4)**: api **1975** (+2 từ 1973 — boss override + BEAST fallback regression), shared **1353** (+13 từ 1340 — items-monster-loot.test.ts integrity + roll), web **1159** unchanged, total **4487 vitest**. E2E golden-path **22 spec** unchanged. Smoke scripts **28 module** unchanged. Local verification PASS (shared typecheck + 1353/1353 + items-monster-loot 13/13 + items-dungeon-loot 24/24 + dungeons-balance 27/27).
- **Top priority next session**: (1) Sửa GitHub Actions runners (infrastructure) — wire `E2E_FULL=1` matrix sau khi CI runners healthy; (2) smoke:daily-login multi-day positive (Small); (3) Concurrency tests phase 2 remainder. Detail §6.
...
| PR | Title | Type |
|---|---|---|
| **this PR** | `feat(shared,api): Phase 12.4 per-monster MonsterDef.lootTable polish — MonsterDef.lootTable?: readonly LootEntry[] (optional override) + rollMonsterLoot(monsterKey, n) helper + DungeonRunService.nextEncounter + CombatService WON paths ưu tiên monster.lootTable trước fallback rollDungeonLoot(dungeon.key, n); seed 5 boss/elite override (1 ELITE kim_dieu_thuong_phong + 4 BOSS thuy_thanh_long_vuong/chu_tuoc_huyet_dieu/tho_dia_lao_tu/cuu_la_huyen_quan) bias themed equipment + skill_book pity weight 5-6 + linh_can_dan pity 3 trên endgame; +13 shared test items-monster-loot + 2 API integration test (override + BEAST fallback regression)` | medium shared+api, **no Prisma migration**, **no FE change** |
...
1. **Phase 12.4 per-monster `MonsterDef.lootTable` polish** **CLOSED** ✅ (this PR). `MonsterDef.lootTable?: readonly LootEntry[]` (optional override) + `rollMonsterLoot(monsterKey, n)` helper. `DungeonRunService.nextEncounter` + `CombatService` WON path (cả 2 call site skill + talent) ưu tiên `monster.lootTable` trước fallback `rollDungeonLoot(dungeon.key, n)`. Seed override 5 monster boss/elite: 1 ELITE `kim_dieu_thuong_phong` (kim_son_mach) + 4 BOSS `thuy_thanh_long_vuong` / `chu_tuoc_huyet_dieu` / `tho_dia_lao_tu` / `cuu_la_huyen_quan`. Bias toward themed equipment (weight 5-8) + skill_book pity weight 5-6 (vs dungeon 2-3) + `linh_can_dan` pity weight 3 trên endgame boss `cuu_la_huyen_quan`. **Schema move**: `LootEntry`/`RolledLoot` sang `combat.ts` (avoid circular import) — re-export từ `items.ts` cho backward compat. **Tests**: 13 shared `items-monster-loot.test.ts` (integrity convention gate ELITE/BOSS-only + weight > 0 + qty range + itemKey orphan check + 80% domination cap; rollMonsterLoot unknown/no-override/[]/first/last/qty ∈ range) + 2 API integration (boss override + BEAST fallback regression).

   **Next priority**: GitHub Actions runners infrastructure (matrix `E2E_FULL=1`).
