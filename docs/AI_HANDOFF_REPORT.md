...
- **Current `main` commit**: post PR #446 merged (`feat(api,common,chat): FailoverRateLimiter Redis-down failover + 7 test`) + PR #445 (Cultivation tick CAS guard) + PR #444 (Realtime ban-during-connection + kickUser) + PR #443 (Inventory `revoke()` race fix) + PR #442 (Phase 12.5 dungeon balance tuning) + PR #441 (docs handoff sync post #440) + PR #440 (Phase 12 Story discoverability QuestView dungeon hint) + PR #439 (Story Foundation Late-game encounter wire) + PR #438 (Phase 12.4 per-monster MonsterDef.lootTable polish) + PR #437 (DungeonRun listForUser activeRun fallback + E2E spec #22). Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), Phase 12 Story PR-1→PR-6 + Foundation Extension + Late-game wire all **CLOSED** ✅ (#425–#433), **Phase 12.2.B/C/12.3** **CLOSED** ✅ (#434–#436), **E2E spec #22 + listForUser fallback** **CLOSED** ✅ (#437), **Phase 12.4 per-monster MonsterDef.lootTable polish** **CLOSED** ✅ (#438), **Story Foundation Late-game encounter wire** **CLOSED** ✅ (#439), **Phase 12 Story discoverability — QuestView dungeon hint cho kill+monster step** **CLOSED** ✅ (#440), **docs handoff sync post #440** **CLOSED** ✅ (#441), **Phase 12.5 dungeon balance tuning** **CLOSED** ✅ (#442), **Concurrency phase 2 — Inventory `revoke()` race fix** **CLOSED** ✅ (#443), **Concurrency phase 2 — Realtime ban-during-connection hardening + kickUser** **CLOSED** ✅ (#444), **Concurrency phase 2 — Cultivation tick CAS guard** **CLOSED** ✅ (#445), **Concurrency phase 2 — Chat Redis failover (FailoverRateLimiter wrap)** **CLOSED** ✅ (#446), **Concurrency phase 2 — Boss spawn cron auto race fix (partial unique index + P2002 graceful + in-process re-entry guard)** **CLOSED** ✅ (this PR — final remainder). Concurrency phase 2 **DONE**.
- **In-flight**: this PR (Concurrency phase 2 final — Boss spawn cron auto race fix via partial unique index `WorldBoss_status_active_unique` + P2002 graceful no-op in `spawnNew()` + in-process `heartbeatRunning` re-entry guard).
- **Test baseline (post this PR)**: api **1995** (+4: 4 BossService spawn-cron concurrency tests — 5× spawnNew exclusivity, spawnNew vs existing ACTIVE, 2× heartbeat exclusivity, adminSpawn force-race vs spawnNew). Carryover #446 chat failover. shared **1370**, web **1163**, total **4528 vitest**. E2E golden-path **22 spec**. Smoke scripts **28 module**.
- **Top priority next session**: (1) Shop daily limit + rate-limit per user (M10) Medium PR; (2) CSP production verify khi deploy (M7); (3) Phase 12 Story dialogue branch / DungeonTemplate story-instance (optional). Detail §6.
...
| PR | Title | Type |
|---|---|---|
| this PR | `fix(api,boss): spawn cron auto race — partial unique index `WorldBoss_status_active_unique` (Prisma migration `20260522000000_concurrency_boss_active_unique`) enforces ≤1 ACTIVE row at any time; `spawnNew()` catches Prisma `P2002` on the active partial unique index → returns `null` (benign no-op) instead of crashing the heartbeat tick; `adminSpawn(force=true)` race-loses → throws `BOSS_ALREADY_ACTIVE` (KHÔNG ghi audit log nói dối là admin spawned). Plus in-process `heartbeatRunning` re-entry guard (skip overlap nếu previous tick còn distribute reward). Closes Concurrency phase 2 final remainder (Inventory `use()`/`revoke()`, Realtime ban, Cultivation tick CAS, Chat Redis failover all CLOSED #422/#443/#444/#445/#446). +4 concurrency test (5× spawnNew exclusivity, spawnNew when ACTIVE pre-exists, 2× heartbeat exclusivity, adminSpawn force-race vs spawnNew).` | small-medium api fix + 1 Prisma migration + 4 tests, no UI change |
...
|---|---|---|
| `apps/api` | **1995** | +4 từ Concurrency phase 2 Boss spawn cron auto race (`boss.service.concurrency.test`: 5× concurrent `spawnNew` → 1 ACTIVE + 4 race-lost null; `spawnNew` khi đã có ACTIVE → null không corrupt; 2× concurrent `heartbeat` → 1 ACTIVE; `adminSpawn(force=true)` race vs `spawnNew` → admin throws `BOSS_ALREADY_ACTIVE` + KHÔNG ghi audit log nói dối). Carryover +7 #446 chat failover. |
| `packages/shared` | **1370** | Carryover từ #442 (Phase 12.5 shared catalog tuning). |
| `apps/web` | **1163** | Carryover từ #440 (no UI change). |
| **Total** | **4528 vitest** | All green local. **GitHub CI runners**: verify khi PR push. |
...
1. **Concurrency phase 2 — DONE** ✅. All 6 races fixed: Inventory `use()` (#422), Inventory `revoke()` (#443), Realtime ban-during-connection (#444), Cultivation tick CAS (#445), Chat Redis failover (#446), **Boss spawn cron auto race (this PR — final remainder; partial unique index `WorldBoss_status_active_unique` + `spawnNew()` P2002 graceful no-op + in-process `heartbeatRunning` re-entry guard + 4 concurrency test)**. Phase đóng — không còn race outstanding cần fix theo audit hiện tại.
