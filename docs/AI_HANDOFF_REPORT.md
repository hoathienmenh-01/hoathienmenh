...
- **Current `main` commit**: `32675b3` post PR #414 merged (Phase 11 nâng cao §5 PR2 prep — `tam_ma_light` buff catalog + `cultivation_rate_mul` BuffEffectKind + `evaluateBreakthroughOutcome` helper + `BreakthroughAttemptLog` Prisma model). Pre-PR `c98a465` post #413 (Phase 11 nâng cao §5 PR1 — `computeBreakthroughChance` formula). Pre-pre-PR `6a44759` post #412 (Phase 11.6.E FE tooltip render). **In-flight this PR**: Phase 11 nâng cao §5 PR2 wire — new `CharacterService.attemptBreakthrough(userId, rng?, now?, idempotencyKey?)` method consume `computeBreakthroughChance` + `evaluateBreakthroughOutcome` + RNG roll [0,1); SUCCESS path advance realm + restats + INSERT `BreakthroughAttemptLog{success:true}` + auto-unlock title (fail-soft) + BREAKTHROUGH achievement track (fail-soft); FAIL path KHÔNG advance + KHÔNG trừ EXP + apply `tam_ma_light` qua `BuffService.applyBuffTx` (atomic cùng tx) + INSERT `BreakthroughAttemptLog{success:false, tamMaActive:true}`. New endpoint `POST /character/breakthrough/attempt` (forward-compat, endpoint cũ `POST /character/breakthrough` deterministic giữ nguyên cho client cũ). Wire `BuffMods.cultivationRateMul` (×0.7 trong 300s) vào `CultivationProcessor.process()` EXP gain composer multiplicatif sau talentExpMul.
- **Current phase**: Phase 10 Content scale **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅ (catalog 11/11 + runtime persistence 10/10 + UI E2E spec #19 merged via #394). **Phase 11 nâng cao** (post-Phase-11 polish — 6 modules theo `XuanToi_Phase11_NangCao_Report.docx`): §6 Balance dial registry **CLOSED** ✅ (PR #398). §3 Elemental Combat MVP **CLOSED** ✅ (PR #399). §2 Skill Ngũ Hành expansion **CLOSED** ✅ (PR #400). **Phase 11.6.C Spiritual Root × Tribulation element resist wire CLOSED** ✅ (PR #401). **Phase 11.1.E Cultivation Method element affinity wire CLOSED** ✅ (PR #405 backend + PR #408 FE badge). **Phase 11.6.D Talent passive element_resist wire CLOSED** ✅ (PR #409). **Phase 11.6.E Equipment elemental resist runtime CLOSED** ✅ (PR #411). **Phase 11.6.E FE tooltip render CLOSED** ✅ (PR #412). **Phase 11 nâng cao §5 PR1 CLOSED** ✅ (PR #413). **Phase 11 nâng cao §5 PR2 prep CLOSED** ✅ (PR #414 — buff catalog + `evaluateBreakthroughOutcome` helper + Prisma audit log model). **Phase 11 nâng cao §5 PR2 wire IN-FLIGHT** (this PR — `CharacterService.attemptBreakthrough` method + `POST /character/breakthrough/attempt` endpoint + `CultivationProcessor.cultivationRateMul` wire). Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** via #397; Phase 12.2 DungeonTemplate/DungeonRun runtime is next. Admin seed harness đầy đủ 8 endpoint. Smoke scripts **26 module** complete. Detail ở `## 3. Current Phase Status`.
- **Test baseline (post PR #414 merged + Phase 11 nâng cao §5 PR2 wire this PR)**: shared **1276/1276** (unchanged baseline post #414) + api **1809/1809** (+9 new — `character.service.test.ts` +6 cho `attemptBreakthrough` SUCCESS/FAIL/NOT_AT_PEAK×2/attemptIndex/legacy-no-buff; `cultivation.processor.test.ts` +3 cho `cultivationRateMul` wire ×0.7/identity/no-buffservice từ baseline 1800) + web **1034/1034** (unchanged — không touch FE). 5 redis-dependent test (rate-limiter + health controller) cần Redis local — pass khi Redis container up. Smoke scripts **26 module** complete. Detail ở `## 5. Tests`.
- **Open PR / pending branch**: 1 in-flight Phase 11 nâng cao §5 PR2 wire PR (this PR — service method + endpoint + cultivation processor wire + tests). PR #414 + #413 + #412 + #411 + #410 + #409 + #408 + #407 + #406 + #405 đã merged. Older docs/audit in-flight (session 5/6 + 5/7) chưa rebase — xem GitHub PR list ở `https://github.com/hoathienmenh-01/xuantoi/pulls`.
...
- **Immediate next task** (3-5 ưu tiên cao nhất theo SESSION PR LIMIT + GOM TRƯỚC KHI TÁCH 4b — Medium PR > Hotfix > Large):
  1. **Phase 11 nâng cao §5 PR2 wire this PR land** (in-flight) — `CharacterService.attemptBreakthrough` method + `POST /character/breakthrough/attempt` endpoint + `BuffMods.cultivationRateMul` wire vào `CultivationProcessor`. Forward-compat (endpoint cũ deterministic giữ nguyên). Tests +9 (api 6 service + 3 processor).
  ...
  3. **smoke:tribulation HTTP coverage** — Small PR, Template B. 19-step negative-path smoke cho `POST /api/character/tribulation` + `GET /api/character/tribulation/log` mirror pattern `smoke:breakthrough`. KHÔNG cần admin seed (gate fail trước khi simulation).
...
| PR | Title | Type |
|---|---|---|
| (in-flight) | `feat(api): Phase 11 nâng cao §5 PR2 wire — CharacterService.attemptBreakthrough + POST /character/breakthrough/attempt + CultivationProcessor cultivationRateMul wire` | medium api wire + tests |
