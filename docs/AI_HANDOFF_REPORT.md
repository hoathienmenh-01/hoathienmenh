...
- **Current `main` commit**: `83eaf02` post PR #408 merged (Phase 11.1.E FE affinity badge render — `+10%`/`+5%` badge consume `equippedMethodElementAffinity` field expose từ #405). Pre-PR `c83c2c0` post #407 (CI hotfix dập 2 text-level warning eslint typeless + Vue router-link). Pre-pre-PR `c08381a` post #406 (Node 24 action bump). **In-flight this PR**: Phase 11.6.D — Talent passive `element_resist` wire vào TribulationService (Medium PR follow-up cùng module với #401 Phase 11.6.C). Compose multiplicatively trên top spiritual root resist: `effective = computeSpiritualRootTribulationResist(...) × computePassiveTalentTribulationResist(...)`, clamp `[ELEMENT_MODIFIER_ABSOLUTE_FLOOR=0.6, ELEMENT_MODIFIER_ABSOLUTE_CEIL=1.5]`. Catalog +5 talent `talent_<elem>_thien_giap` (kim/moc/thuy/hoa/tho, kind=`element_resist` value=`0.95`, cost=1, realmRequirement=`kim_dan`). Shared `PassiveTalentMods` thêm `elementResistByElement: ReadonlyMap<ElementKey, number>` field + `composePassiveTalentMods` handle kind mới + `computePassiveTalentTribulationResist` pure helper. `BALANCE_DIALS.TALENT_ELEMENT_RESIST_VALUE=0.95` single source-of-truth. `TribulationService` constructor `@Optional() talents?: TalentService` (backward-compat — legacy/test bootstrap chưa inject → fallback identity 1.0). FE TalentCatalogView effectSummary thêm case `'element_resist'` + i18n `talents.effect.elementResist` ("-{pct}% sát thương từ hệ {element}" vi/en). Server-authoritative — `attemptTribulation` fetch `talents.getMods(characterId)` rồi compose vào `elementResistFn` closure trước khi `simulateTribulation`.
- **Current phase**: Phase 10 Content scale **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅ (catalog 11/11 + runtime persistence 10/10 + UI E2E spec #19 merged via #394). **Phase 11 nâng cao** (post-Phase-11 polish — 6 modules theo `XuanToi_Phase11_NangCao_Report.docx`): §6 Balance dial registry **CLOSED** ✅ (PR #398). §3 Elemental Combat MVP **CLOSED** ✅ (PR #399). §2 Skill Ngũ Hành expansion **CLOSED** ✅ (PR #400). **Phase 11.6.C Spiritual Root × Tribulation element resist wire CLOSED** ✅ (PR #401). **Phase 11.1.E Cultivation Method element affinity wire CLOSED** ✅ (PR #405 backend + PR #408 FE badge). **Phase 11.6.D Talent passive element_resist wire IN-FLIGHT** (this PR — compose multiplicatively trên spiritual root resist via `composePassiveTalentMods` element_resist kind + `computePassiveTalentTribulationResist`, +14 shared unit tests + 5 API integration tests). Equipment-layer resist (Phase 11.6.E) defer riêng (cần Equipment Module runtime, lớn hơn). Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** via #397; Phase 12.2 DungeonTemplate/DungeonRun runtime is next. Admin seed harness đầy đủ 8 endpoint. Smoke scripts **26 module** complete. Detail ở `## 3. Current Phase Status`.
- **Test baseline (post PR #408 merged + Phase 11.6.D this PR)**: api **1788/1788 vitest** (+5 new — `TribulationService.attemptTribulation — talent resist (Phase 11.6.D)` describe block: TalentService NOT injected backward-compat / TalentService injected + no talent learned / `talent_hoa_thien_giap` reduce 2 hoa wave 5% / `talent_kim_thien_giap` reduce kim wave 5% / compose 2 talent + spiritual root primary=hoa edge case `0.9 × 0.95`, từ baseline 1782) + shared **1190/1190** (+14 new — 1 invariant test cho `element_resist` kind + 13 tests cho `composePassiveTalentMods` element_resist Map composition + `computePassiveTalentTribulationResist` helper, từ baseline 1176) + web **1031/1031** (unchanged — TalentCatalogView display-only thay đổi, existing tests cover render path). 5 redis-dependent test (rate-limiter + health controller) cần Redis local — pass khi Redis container up. Smoke scripts **26 module** complete. `pnpm test:balance` pass với `talentResist.elementResistValue` snapshot field mới. Detail ở `## 5. Tests`.
- **Open PR / pending branch**: 1 in-flight Phase 11.6.D Talent element_resist PR (this PR). PR #408 + #407 + #406 + #405 + #404 + #403 + #402 + #401 đã merged. Older docs/audit in-flight (session 5/6 + 5/7) chưa rebase — xem GitHub PR list ở `https://github.com/hoathienmenh-01/xuantoi/pulls`.
...
- **Immediate next task** (3-5 ưu tiên cao nhất theo SESSION PR LIMIT + GOM TRƯỚC KHI TÁCH 4b — Medium PR > Hotfix > Large):
  1. **Phase 11.6.D Talent passive `element_resist` this PR land** (in-flight) — Compose talent layer trên top spiritual root resist trong TribulationService, +5 catalog talent `talent_<elem>_thien_giap`, +14 shared unit tests + 5 API integration tests + FE i18n. Backward-compat `@Optional() TalentService` injection.
  2. **Phase 11.6.E Equipment elemental resist layer (next)** — Compose equipment resist trên top spiritual root + talent. Cần Equipment Module runtime đầu tiên (Phase 11.5 Refine). **Risk**: Medium-Large — depends on Equipment Module persistence + slot system. Wire pattern mirror Phase 11.6.D (talent layer): `equipment.getResistMods(characterId) → ReadonlyMap<ElementKey, number>`, multiply vào `elementResistFn` closure, clamp envelope đã có.
...
| PR | Title | Type |
|---|---|---|
| (in-flight) | `docs(handoff): split archive section into ARCHIVE_HANDOFF.md để giảm token cost mỗi session` | docs compact |
| (in-flight) | `feat(api,shared,web): Phase 11.6.D — Talent passive element_resist wire vào TribulationService + 5 talent_*_thien_giap catalog` | medium feature + tests |
...
| [#400](https://github.com/hoathienmenh-01/xuantoi/pull/400) | `feat(skills,catalog): Phase 11 nâng cao §2 — Skill Ngũ Hành expansion (+10 skill nguyen_anh ULT + truc_co role-fill)` | catalog static + invariant test |
| [#399](https://github.com/hoathienmenh-01/xuantoi/pull/399) | `feat(combat,balance): Phase 11 nâng cao §3 — Elemental Combat MVP polish (centralize matrix + breakdown logging)` | combat tuning + tests |
| [#398](https://github.com/hoathienmenh-01/xuantoi/pull/398) | `feat(balance,catalog): Phase 11 nâng cao §6 — balance dial registry + pnpm test:balance` | catalog static + invariant |
| [#397](https://github.com/hoathienmenh-01/xuantoi/pull/397) | `feat(map-region,catalog): Phase 12.1 MapRegion static catalog foundation (9 region)` | catalog static + cross-catalog ref |
...
1. **Phase 11.1.E FE badge render** — Small PR, Template A (FE polish). Hiện `CultivationMethodStateOut.equippedMethodElementAffinity` đã expose qua API nhưng UI chưa render badge. Setup:
   - **API client**: bổ sung `equippedMethodElementAffinity` vào `apps/web/src/api/cultivation-method.ts` response type.
   - **Pinia store**: extend `useCultivationMethodStore` getters expose `affinityPercentLabel` ("+10%" / "+5%" / "0%").
   - **View**: thêm badge ở `apps/web/src/views/CultivationMethodView.vue` cạnh equipped method name (chỉ render khi `> 0`).
   - **i18n**: 2 key `cultivation.method.affinity.primary` / `cultivation.method.affinity.secondary`.
   - **Tests**: store snapshot + view conditional render.
