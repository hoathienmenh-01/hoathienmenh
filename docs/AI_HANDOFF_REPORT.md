...
- **Current `main` commit**: `c98a465` post PR #413 merged (Phase 11 nâng cao §5 PR1 — `computeBreakthroughChance` shared formula + 4 dial mới + 33 unit tests). Pre-PR `6a44759` post #412 (Phase 11.6.E FE tooltip render). Pre-pre-PR `5070529` post #411 (Phase 11.6.E Equipment elemental tribulation resist runtime). **In-flight this PR**: Phase 11 nâng cao §5 PR2 prep — `tam_ma_light` buff catalog entry trong `packages/shared/src/buffs.ts` (source `breakthrough`, polarity `debuff`, duration 300s, `cultivation_rate_mul ×0.7` non-stackable, non-dispellable) + new `cultivation_rate_mul` BuffEffectKind composition multiplicatif vào `BuffMods.cultivationRateMul` + new `breakthrough` BuffSource + new `evaluateBreakthroughOutcome` pure helper trong `packages/shared/src/breakthrough-chance.ts` (deterministic gate fail / success / fail+debuff outcome từ breakdown + rngRoll [0,1) + now date) + new Prisma `BreakthroughAttemptLog` model (audit log full breakdown 4 layer + rngRoll + success + expBefore/After + tâm ma fields + idempotencyKey + attemptIndex, FK Character cascade). Forward-compat — schema/catalog ready nhưng `BreakthroughService.attempt()` chưa wire INSERT/buff (PR2 wire kế tiếp consume). KHÔNG có API/runtime thay đổi.
- **Current phase**: Phase 10 Content scale **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅ (catalog 11/11 + runtime persistence 10/10 + UI E2E spec #19 merged via #394). **Phase 11 nâng cao** (post-Phase-11 polish — 6 modules theo `XuanToi_Phase11_NangCao_Report.docx`): §6 Balance dial registry **CLOSED** ✅ (PR #398). §3 Elemental Combat MVP **CLOSED** ✅ (PR #399). §2 Skill Ngũ Hành expansion **CLOSED** ✅ (PR #400). **Phase 11.6.C Spiritual Root × Tribulation element resist wire CLOSED** ✅ (PR #401). **Phase 11.1.E Cultivation Method element affinity wire CLOSED** ✅ (PR #405 backend + PR #408 FE badge). **Phase 11.6.D Talent passive element_resist wire CLOSED** ✅ (PR #409). **Phase 11.6.E Equipment elemental resist runtime CLOSED** ✅ (PR #411). **Phase 11.6.E FE tooltip render CLOSED** ✅ (PR #412). **Phase 11 nâng cao §5 PR1 CLOSED** ✅ (PR #413 — `computeBreakthroughChance` formula). **Phase 11 nâng cao §5 PR2 prep IN-FLIGHT** (this PR — `tam_ma_light` buff catalog + `cultivation_rate_mul` effect kind + `evaluateBreakthroughOutcome` helper + `BreakthroughAttemptLog` Prisma model migration). Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** via #397; Phase 12.2 DungeonTemplate/DungeonRun runtime is next. Admin seed harness đầy đủ 8 endpoint. Smoke scripts **26 module** complete. Detail ở `## 3. Current Phase Status`.
- **Test baseline (post PR #413 merged + Phase 11 nâng cao §5 PR2 prep this PR)**: shared **1276/1276** (+25 new — `buffs.test.ts` 59→66 tests +7 cho tam_ma_light catalog + cultivationRateMul composition; `breakthrough-chance.test.ts` 30→48 tests +18 cho `evaluateBreakthroughOutcome` gate-fail/success/fail+debuff/RNG-validation/determinism, từ baseline 1251 post #413) + api **1800/1800** (1812 vitest pre-PR, +0 net — combat.service.test.ts 2 spy mock thêm field `cultivationRateMul: 1` để khớp BuffMods interface mở rộng; còn lại unchanged) + web **1034/1034** (unchanged — không touch FE). 5 redis-dependent test (rate-limiter + health controller) cần Redis local — pass khi Redis container up. Migration `20260514000000_phase_11_nang_cao_5_pr2_breakthrough_attempt_log` test reset+deploy local OK. Smoke scripts **26 module** complete. Detail ở `## 5. Tests`.
- **Open PR / pending branch**: 1 in-flight Phase 11 nâng cao §5 PR2 prep PR (this PR — buff catalog + outcome helper + Prisma migration). PR #413 + #412 + #411 + #410 + #409 + #408 + #407 + #406 + #405 + #404 đã merged. Older docs/audit in-flight (session 5/6 + 5/7) chưa rebase — xem GitHub PR list ở `https://github.com/hoathienmenh-01/xuantoi/pulls`.
...
- **Immediate next task** (3-5 ưu tiên cao nhất theo SESSION PR LIMIT + GOM TRƯỚC KHI TÁCH 4b — Medium PR > Hotfix > Large):
  1. **Phase 11 nâng cao §5 PR2 prep this PR land** (in-flight) — `tam_ma_light` buff catalog + `cultivation_rate_mul` BuffEffectKind + `breakthrough` BuffSource + `evaluateBreakthroughOutcome` helper + `BreakthroughAttemptLog` Prisma model. Forward-compat schema/helper ready, KHÔNG wire runtime — PR2 wire (`BreakthroughService.attempt()` consume) là step kế.
  2. **Phase 11 nâng cao §5 PR2 wire — Tâm Ma debuff runtime** — Medium PR, Template C. Wire `BreakthroughService.attempt()` consume `computeBreakthroughChance` + `evaluateBreakthroughOutcome` + deterministic RNG (per-character seed). Success advance realm. Fail apply `tam_ma_light` qua `BuffService.applyBuff` + INSERT `BreakthroughAttemptLog` row. Idempotency key per attempt. Cultivation EXP gain composer dùng `BuffMods.cultivationRateMul` để áp 0.7× penalty trong 300s.
  3. **Phase 11 nâng cao §5 PR3 — Breakthrough UI history** — Medium PR, Template B. Cultivation view → "Đột phá" button + chance breakdown tooltip + history list từ `BreakthroughAttemptLog`. E2E spec verify success/fail/Tâm Ma debuff visible.
  4. **smoke:tribulation HTTP coverage** — Small PR, Template B. 19-step negative-path smoke cho `POST /api/character/tribulation` + `GET /api/character/tribulation/log` mirror pattern `smoke:breakthrough`. KHÔNG cần admin seed (gate fail trước khi simulation).
  5. **Phase 12.2 DungeonTemplate + DungeonRun runtime** — Medium PR, Template C. Prisma model `DungeonTemplate` + `DungeonRun` + service `startRun`/`nextEncounter`/`claimRun` happy-path + Prisma migration. **Risk**: Prisma migration + new module.
...
| PR | Title | Type |
|---|---|---|
| (in-flight) | `feat(shared,api): Phase 11 nâng cao §5 PR2 prep — tam_ma_light buff catalog + cultivation_rate_mul effect + evaluateBreakthroughOutcome + BreakthroughAttemptLog Prisma model` | medium shared+Prisma prep + tests |
