...
- **Current `main` commit**: post PR #456 merged (`fix(api,web,shared): post-Phase-13.0 audit pass #5 — render rewardHintI18nKey + console.error cleanup`) + Phase 13.0 LiveOps & Retention Suite + PR #451 (Phase 11.10.E Pill→Buff) + PR #450 (M10 Shop daily limit) + ... Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **CLOSED** ✅ (#397–#448). **Phase 13.0 LiveOps & Retention Suite** **CLOSED** ✅ (post #456 audit pass #5). **Phase 13.1.A Sect War Core / Contribution / Leaderboard / Weekly Reward** **OPEN** — this PR.
- **In-flight**: this PR — **Phase 13.1.A Sect War Core**. Shared: `sect-war.ts` catalog (5 activity + 4 reward tier), helpers `sectWarWeekKey()`/`currentSectWarSeason()`/`sectWarActivityByKey()`/`sectWarRewardTierForRank()`; title +1 `sect_war_champion`. Prisma migration `20260524000000_phase_13_1_a_sect_war_core`: `SectWarContribution` + `SectWarWeeklyRewardClaim` (composite UNIQUE chống double-add/double-claim). API: `SectWarModule` + 4 endpoint `/sect-war/{current,leaderboard,me,claim}`. 5 hook gameplay (DungeonRun.claim / Boss.distributeRewards participation+topDamage / DailyLogin.claim / Quest.claim) defensive try-catch — character không sect → no-op. FE: `/sect-war` route + 4 panel; HomeView CTA "Tông Môn Chiến"; i18n vi/en đầy đủ. KHÔNG đụng Sect Missions / Sect Shop / Admin LiveOps Controls (deferred 13.1.B).
- **Test baseline (post this PR)**: shared **+38** sect-war (catalog invariants, weekKey/timezone, reward tier). api **+14** sect-war service (contribution idempotency, cap enforcement, leaderboard ordering, claim race, error mapping). web **+8** SectWarView + **+1** LiveOpsTodayPanel CTA + **+1** routes manifest. Cumulative **shared 1463 / api 2077 (6 pre-existing Redis fail) / web 1249**. E2E golden-path **22 spec**. Smoke scripts **28 module**.
...
| PR | Title | Type |
|---|---|---|
| this PR | `feat(api,web,shared): Phase 13.1.A Sect War Core — Tông Môn Chiến tuần lễ. Shared `sect-war.ts` catalog: 5 activity (`daily_login` 5pt cap 7/tuần, `dungeon_clear` 10pt cap 50/ngày, `boss_participation` 15pt cap 120/tuần, `boss_top_damage` 25pt cap 100/tuần, `quest_complete` 8pt cap 80/tuần) + 4 reward tier (rank 1 / 2-3 / 4-10 / participation), helpers `sectWarWeekKey()`/`currentSectWarSeason()`/`sectWarActivityByKey()`/`sectWarRewardTierForRank()` (timezone reuse `MISSION_RESET_TZ`, season Thứ Hai 00:00 ICT → Chủ Nhật 23:59 ICT); title +1 `sect_war_champion` (epic). Prisma migration `20260524000000_phase_13_1_a_sect_war_core`: 2 table mới `SectWarContribution` (composite UNIQUE `(weekKey, sourceType, sourceId, characterId)` chống double-add) + `SectWarWeeklyRewardClaim` (composite UNIQUE `(weekKey, characterId)` chống double-claim). API: `SectWarService` + `SectWarController` + 4 endpoint `/sect-war/{current,leaderboard,me,claim}` (Envelope, atomic CAS qua composite UNIQUE — race-safe). 5 hook gameplay defensive try-catch: `DungeonRunService.claim`, `BossService.distributeRewards` (participation + topDamage), `DailyLoginService.claim`, `QuestService.claim` — character không có sect → no-op (return null). FE: `/sect-war` route + `SectWarView.vue` + 4 panel (Leaderboard, MyProgress, ActivityRules, Reward); HomeView LiveOps panel thêm CTA "Tông Môn Chiến"; i18n vi/en đầy đủ (`sectWar.*` + 5 error code + 5 activity label/desc + 4 tier label/desc); claim button gating (`canClaim` = hasSect + eligibleTier + !alreadyClaimed). Tests: shared **+38** (catalog invariants, weekKey/timezone, reward tier); api **+14** sect-war service (contribution idempotency, cap enforcement, leaderboard ordering, claim race, error mapping); web **+8** SectWarView + **+1** LiveOpsTodayPanel CTA + **+1** routes manifest. Verification: shared 1463 ✅ / web 1249 ✅ / api 2077 ✅ (6 fail Redis-dependent rate-limiter + health controller — pre-existing env limitation, confirmed identical fail trên `main` mà KHÔNG có changes); typecheck shared/api/web ✅; pnpm build ✅. KHÔNG đụng Sect Missions / Sect Shop / Admin LiveOps Controls (deferred Phase 13.1.B).` | feat |
