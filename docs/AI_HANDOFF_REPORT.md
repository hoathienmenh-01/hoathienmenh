...
- **Current `main` commit**: post PR #429 merged (`feat(shared): Phase 12 Story Foundation Extension — Kim Đan + Nguyên Anh catalog +10 quest + 1 NPC + 5 dialogue line + integrity test`) + PR #428 (Phase 12 PR-4 NPC dialogue UI) + PR #427 (Phase 12 PR-3 Quest claim) + PR #426 (Phase 12 PR-2 Quest runtime persistence) + PR #425 (Phase 12 PR-1 catalog foundation). Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), **Phase 12 Story PR-1** catalog **CLOSED** ✅ (#425), **Phase 12 Story PR-2** Quest runtime persistence **CLOSED** ✅ (#426), **Phase 12 Story PR-3** Quest claim/reward idempotency **CLOSED** ✅ (#427), **Phase 12 Story PR-4** NPC dialogue UI **CLOSED** ✅ (#428), **Phase 12 Story Foundation Extension** Kim Đan + Nguyên Anh catalog **CLOSED** ✅ (#429). **Phase 12 Story Runtime MVP** Quest UI (QuestView.vue + Pinia store + accept/claim consume PR-2/3 endpoints) **OPEN** via this PR. Main storyline Chapter 1 playable (PR-5) chưa start.
- **In-flight**: this PR = Phase 12 Story Runtime MVP (Quest UI: `QuestView.vue` + Pinia store `useQuestStore` + 3 typed API wrappers + router `/quests` + nav AppShell + i18n vi/en parity; consume `GET /quests/me` + `POST /quests/accept` + `POST /quests/claim` PR-2/3 sẵn có, KHÔNG endpoint mới, KHÔNG Prisma migration).
- **Test baseline (post Phase 12 Story Runtime MVP)**: api **1907**, shared **1328** (PR #429 catalog extension đã merged), web **1126** (+25 from `stores/__tests__/quest.test.ts` 12 + `views/__tests__/QuestView.test.ts` 13), total **4361 vitest**. E2E golden-path **20/20 spec**. Smoke scripts **27 module**. CI expected GREEN.
...
| PR | Title | Type |
|---|---|---|
| **this PR** | `feat(web): Phase 12 Story Runtime MVP — QuestView.vue list + filter + accept/claim UI (Pinia store useQuestStore + 3 typed API wrappers + router /quests + nav AppShell + i18n vi/en parity + 25 new test) consume PR-2/3 endpoints, server-authoritative` | medium FE feature + tests, KHÔNG endpoint/Prisma mới |
