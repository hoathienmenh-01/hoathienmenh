...
- **Current `main` commit**: post PR #460 merged (`test(api): add daily login multi-day smoke positive coverage`) + PR #459 (Phase 13.1.B Sect Missions + Sect Shop + Admin LiveOps Controls) + PR #458 (post Phase 13.1.A hardening audit) + PR #457 (Phase 13.1.A Sect War Core) + PR #456 (post-Phase-13.0 audit pass #5) + Phase 13.0 LiveOps & Retention Suite + PR #451 (Phase 11.10.E Pill→Buff) + PR #450 (M10 Shop daily limit) + ... Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **CLOSED** ✅ (#397–#448). Phase 13.0 LiveOps & Retention Suite **CLOSED** ✅ (post #456 audit pass #5). **Phase 13.1.A Sect War Core / Contribution / Leaderboard / Weekly Reward** **CLOSED** ✅ (PR #457 merged + PR #458 hardening audit). **Phase 13.1.B Sect Missions + Sect Shop + Admin LiveOps Controls** **CLOSED** ✅ (PR #459 merged). **Daily-login multi-day smoke positive** **CLOSED** ✅ (PR #460 merged).
- **In-flight**: none — last merged là PR #460 (daily-login multi-day smoke positive: 5 case integration test cho `DailyLoginService.claim(userId, now)` qua test-clock injection — chuỗi 7 ngày, double-claim idempotent, missed-day reset streak, timezone boundary VN ICT).
- **Test baseline (post PR #460)**: shared **1463**. api **2123** (+5 từ multi-day smoke test). web **1265**. E2E golden-path **22 spec**. Smoke scripts **28 module**. Pre-existing flaky `chat.service.test.ts` rate-limit test (timing window) pass on retry.
- **Top priority next session**: (1) Phase 12 Story dialogue branch / story dungeon instance; (2) CSP production verify khi deploy (M7); (3) Admin LiveOps advanced controls (manual force-spawn boss override, season schedule editor); (4) Phase 13.2 Cross-sect seasonal expansion / auction / diplomacy. Detail §6.
...
### Vitest baseline (post PR #460)

| Workspace | Test count | Notes |
|---|---|---|
| `apps/api` | **2123** | +5 từ PR #460 (`daily-login.multi-day.test.ts`) lên trên 2118 baseline post Phase 13.1.B (SectMissionService 12 + SectShopService 12 + AdminLiveOpsService 10 + 6 integration). Pre-existing `chat.service.test.ts` rate-limit window test occasionally flakes khi chạy parallel; pass cô lập (`pnpm --filter @xuantoi/api test -- --run chat.service`). 6 redis-dependent tests (rate-limiter + health) require Redis up. |
| `packages/shared` | **1463** | Không thay đổi vs PR #457/#459/#460. |
| `apps/web` | **1265** | +16 từ Phase 13.1.B: SectMissionPanel (5) + SectShopPanel (7) + AdminLiveOpsPanel (4) + 8 SectWarView migration test. PR #460 không đổi web. |
| **Total** | **4851 vitest** | All green local (PG+Redis up). |
