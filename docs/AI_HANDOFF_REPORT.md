...
- **Current `main` commit**: post PR #433 merged (`feat(shared): Phase 12 Story Foundation Late-game wire` — 8 monster catalog + 2 invariant test) + PR #432 merged (`fix(api,shared): Phase 12 Story PR-6 — Combat kill hook → quest track auto-wire`) + PR #431 (Phase 12 Story PR-5 Main storyline Chapter 1 playable) + PR #430 (Phase 12 Story Runtime MVP QuestView UI) + PR #429 (Phase 12 Story Foundation Extension Kim Đan + Nguyên Anh catalog) + PR #428 (Phase 12 PR-4 NPC dialogue UI) + PR #427 (Phase 12 PR-3 Quest claim) + PR #426 (Phase 12 PR-2 Quest runtime persistence) + PR #425 (Phase 12 PR-1 catalog foundation). Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), **Phase 12 Story PR-1** catalog **CLOSED** ✅ (#425), **Phase 12 Story PR-2** Quest runtime persistence **CLOSED** ✅ (#426), **Phase 12 Story PR-3** Quest claim/reward idempotency **CLOSED** ✅ (#427), **Phase 12 Story PR-4** NPC dialogue UI **CLOSED** ✅ (#428), **Phase 12 Story Foundation Extension** Kim Đan + Nguyên Anh catalog **CLOSED** ✅ (#429), **Phase 12 Story Runtime MVP** Quest UI **CLOSED** ✅ (#430), **Phase 12 Story PR-5 Main storyline Chapter 1 playable** **CLOSED** ✅ (#431), **Phase 12 Story PR-6 Combat kill hook → quest auto-track** **CLOSED** ✅ (#432), **Phase 12 Story Foundation Late-game wire** **CLOSED** ✅ (#433). **Phase 12.2.B DungeonTemplate + DungeonRun runtime** (this PR) **OPEN**.
- **In-flight**: this PR = Phase 12.2.B DungeonTemplate + DungeonRun runtime. **Context**: PR #421 đã có `DungeonDef.dailyLimit`, PR #433 wire 9 dungeon catalog với `runReward`. Multi-encounter expedition runtime chưa có — player chưa "chơi" được dungeon end-to-end. **Shared**: extend `DungeonDef` với `runReward: { linhThach?, tienNgoc?, exp?, items? }` + 5 invariant test mới (key/monster/items/qty/scale/non-empty). **Prisma**: thêm `DungeonRunStatus` enum (ACTIVE/COMPLETED/CLAIMED/ABANDONED) + `DungeonRun` model (id/characterId/templateKey/status/encounterIndex/killedMonsters JSON/startedAt/completedAt/claimedAt/updatedAt + 3 index) — migration `20260521000000_phase_12_2_b_dungeon_run`. **API**: `DungeonRunModule` mới (service 350+ dòng, controller 120+ dòng) với 4 endpoint server-authoritative: `GET /dungeons/me` list catalog + flag unlocked/dailyUsed/activeRun, `POST /dungeons/:templateKey/start` (realm gate + daily limit + stamina + ALREADY_IN_RUN guard), `POST /dungeon-runs/:runId/next` (CAS guard `encounterIndex` + auto-resolve 1 monster + `QuestService.track('kill','monster',[monster.key, ...questTargetIds])` fail-soft + advance hoặc COMPLETED khi hết list), `POST /dungeon-runs/:runId/claim` (CAS guard `claimedAt: null` qua `updateMany` + `CurrencyService.applyTx` reason `DUNGEON_RUN_REWARD` + `InventoryService.grantTx` + exp increment). **CurrencyService + InventoryService** thêm reason `'DUNGEON_RUN_REWARD'`. **Tests**: api 1969 (+44 — service 21 + controller 23), shared 1340 (+5 runReward invariant từ #433), web 1126 unchanged, total **4435 vitest**. **Smoke**: `scripts/smoke-dungeon-run.mjs` 16 step (auth/no-char/onboard/list/start/invalid/realm-lock/start/next×N/COMPLETED/RUN_NOT_ACTIVE/claim/double-claim) — local PASS 16/16 trên xuantoi-pg. **No FE change** (UI defer sang PR sau khi runtime confirmed).
- **Test baseline (post 12.2.B runtime)**: api **1969** (+44 — `dungeon-run.service.test.ts` 21 + `dungeon-run.controller.test.ts` 23), shared **1340** (+5 runReward invariant từ #433), web **1126** unchanged, total **4435 vitest**. E2E golden-path **21/21 spec** unchanged. Smoke scripts **28 module** (+`smoke:dungeon-run`). Local verification PASS đầy đủ (typecheck 3/3 + lint 3/3 + build 3/3 + smoke 16/16). **GitHub CI hiện không pickup được runners** — verify dựa vào local test mạnh hơn bình thường.
- **Top priority next session**: (1) Phase 12.2.C DungeonRun FE UI (Medium FE — DungeonsView consume `GET /dungeons/me` + start/next/claim flow + claim modal); (2) Sửa GitHub Actions runners (infrastructure — billing/quota check); (3) smoke:daily-login multi-day positive (Small); (4) Concurrency tests phase 2 backlog. Detail §6.
...
|---|---|---|
| `apps/api` | **1969** | +44 từ Phase 12.2.B (`dungeon-run.service.test.ts` 21 + `dungeon-run.controller.test.ts` 23). 1 flaky pre-existing chat rate-limit timing test (50ms sliding window) passes on retry. |
| `packages/shared` | **1340** | +5 runReward invariant từ PR #433. |
| `apps/web` | **1126** | Carryover từ PR #430 (Phase 12 Story Runtime MVP). |
| **Total** | **4435 vitest** | All green local. **GitHub CI runners hiện không pickup được** — verify dựa local. |
...
1. **Phase 12.2.B DungeonTemplate + DungeonRun runtime** **CLOSED** ✅ (this PR). Prisma `DungeonRun` + 4 endpoint server-authoritative (list/start/next/claim) + reward qua `applyTx`/`grantTx` + `QuestService.track` auto-wire khi `nextEncounter` + smoke 16 step + 44 unit/controller test. Daily limit enforcement reuse cùng `startOfLocalDay` helper từ #421.

   **Phase 12.2.C — Next**: DungeonRun FE UI (Medium FE) — `DungeonsView.vue` consume `GET /dungeons/me` + start/next/claim flow + claim reward modal + Pinia store + i18n vi/en + 20+ test. Server-authoritative (FE chỉ render, không tự tăng exp/tiền).
