...
- **Current `main` commit**: post PR #443 merged (`fix(api,inventory): atomic revoke() qty decrement + 2 concurrency tests`) + PR #442 (Phase 12.5 dungeon balance tuning) + PR #441 (docs handoff sync post #440) + PR #440 (Phase 12 Story discoverability QuestView dungeon hint) + PR #439 (Story Foundation Late-game encounter wire) + PR #438 (Phase 12.4 per-monster MonsterDef.lootTable polish) + PR #437 (DungeonRun listForUser activeRun fallback + E2E spec #22) + PR #436 (Phase 12.3 DungeonRun per-encounter loot wire) + PR #435 (Phase 12.2.C DungeonRun FE UI) + PR #434 (Phase 12.2.B DungeonTemplate + DungeonRun runtime). Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), Phase 12 Story PR-1→PR-6 + Foundation Extension + Late-game wire all **CLOSED** ✅ (#425–#433), **Phase 12.2.B/C/12.3** **CLOSED** ✅ (#434–#436), **E2E spec #22 + listForUser fallback** **CLOSED** ✅ (#437), **Phase 12.4 per-monster MonsterDef.lootTable polish** **CLOSED** ✅ (#438), **Story Foundation Late-game encounter wire** **CLOSED** ✅ (#439), **Phase 12 Story discoverability — QuestView dungeon hint cho kill+monster step** **CLOSED** ✅ (#440), **docs handoff sync post #440** **CLOSED** ✅ (#441), **Phase 12.5 dungeon balance tuning** **CLOSED** ✅ (#442), **Concurrency phase 2 — Inventory `revoke()` JS-capture race fix** **CLOSED** ✅ (#443), **Concurrency phase 2 — Realtime ban-during-connection hardening + kickUser** **CLOSED** ✅ (this PR). Next mở: Concurrency phase 2 remainder (Cultivation multi-instance lock).
- **In-flight**: this PR (Concurrency phase 2 — Realtime ban-during-connection hardening).
- **Test baseline (post this PR)**: api **1982** (+5: 2 race gateway-test + 3 kickUser unit), shared **1370**, web **1163**, total **4515 vitest**. E2E golden-path **22 spec**. Smoke scripts **28 module**.
- **Top priority next session**: (1) Concurrency tests phase 2 remainder (Cultivation multi-instance lock — BullMQ Worker pure test backstop); (2) Shop daily limit + rate-limit per user (M10); (3) CSP production verify khi deploy (M7); (4) Phase 12 Story dialogue branch / DungeonTemplate story-instance (optional). Detail §6.
...
| PR | Title | Type |
|---|---|---|
| this PR | `fix(api,realtime): ban during WS connection — RealtimeGateway.handleConnection thiếu `User.banned` check sau JWT verify, banned user vẫn connect/nhận `state:update`/`chat:msg`/`cultivate:tick` đến khi access-token TTL ~15min expire (auth REST path đã chặn login/refresh/me bằng ACCOUNT_BANNED nhưng WS bỏ sót). Fix: post-JWT-verify query `user.banned` → emit error{code:ACCOUNT_BANNED} + disconnect(true), không attach socket. Thêm `RealtimeService.kickUser(userId, reason)` snapshot socket id set + emit + disconnect, idempotent (return false nếu offline / server chưa bind). Wire `AdminService.setBanned(banned=true)` gọi `realtime.kickUser(targetUserId, 'ACCOUNT_BANNED')` sau Prisma update + audit. +2 race test gateway (banned-pre-connect + ban-during-connection) + 3 unit test service.kickUser. Concurrency phase 2 remainder progress.` | small-medium api fix + tests, no Prisma migration, no UI change |
...
|---|---|---|
| `apps/api` | **1982** | +5 từ Concurrency phase 2 ban-during-connection (`realtime.gateway.test` 2 race: banned-pre-connect emit error+disconnect / ban-during-connection kickUser idempotent + `realtime.service.test` 3 unit kickUser: emit+disconnect tất cả socket / offline no-op return false / server chưa bind no-op). Carryover #443. |
| `packages/shared` | **1370** | Carryover từ #442 (Phase 12.5 shared catalog tuning). |
| `apps/web` | **1163** | Carryover từ #440 (no UI change). |
| **Total** | **4515 vitest** | All green local. **GitHub CI runners**: verify khi PR push. |
...
1. **Concurrency tests phase 2 remainder** — Small-Medium PR, Template B per scenario. Inventory `use()` race FIXED via PR #422; Inventory `revoke()` race FIXED via PR #443; **Realtime ban-during-connection FIXED via this PR** (handleConnection `User.banned` check + `RealtimeService.kickUser` + `AdminService.setBanned` wire + 5 test). Còn lại: Cultivation multi-instance lock (Redis lease — BullMQ Worker đã handle nhưng cần test backstop), Chat Redis failover branch, Boss spawn cron auto. Mỗi scenario ~50-150 LOC test.
...
- **Concurrency tests phase 2 (low priority remainder)**: Cultivation multi-instance lock (BullMQ Worker pure test backstop), Chat Redis failover branch, Boss spawn cron auto.
