...
- **Current `main` commit**: post PR #417 merged (smoke RNG positive-path `/breakthrough/attempt` qua admin grant-exp peak seed 25 → 31 step). Pre-PR `1a2ff88` post #416 (smoke negative-path mirror 6 step). Pre-pre-PR `e7bd6a7` post #415 (Phase 11 nâng cao §5 PR2 wire — `CharacterService.attemptBreakthrough` method + `POST /character/breakthrough/attempt` endpoint + `CultivationProcessor.cultivationRateMul` wire ×0.7). **In-flight this PR**: Phase 11 nâng cao §5 PR3 backend prep — expose `BreakthroughAttemptLog` qua `GET /character/breakthrough/log` cho FE history view (mirror `tribulation/log` pattern). Service method `CharacterService.listBreakthroughAttemptLogs(characterId, limit?)` query Prisma sort `createdAt` DESC + cap `BREAKTHROUGH_LOG_MAX_LIMIT=100` + BigInt fields (`expBefore`, `expAfter`) cast → string + Date fields (`tamMaExpiresAt`, `createdAt`) cast → ISO. Controller endpoint `@Get('breakthrough/log')` 404 NO_CHARACTER như cookie session chưa onboard, 401 UNAUTH như chưa register, optional `?limit=N` (1..100) parse safe via `parseBreakthroughLogLimit`. Smoke extend 31 → 34 step cover GET `/breakthrough/log` default + `?limit=1/abc/999` cap + 401 post-logout.
- **Current phase**: Phase 10 Content scale **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅ (catalog 11/11 + runtime persistence 10/10 + UI E2E spec #19 merged via #394). **Phase 11 nâng cao** (post-Phase-11 polish — 6 modules): §6 Balance dial **CLOSED** ✅ (PR #398). §3 Elemental Combat MVP **CLOSED** ✅ (PR #399). §2 Skill Ngũ Hành expansion **CLOSED** ✅ (PR #400). **Phase 11.6.C Spiritual Root × Tribulation element resist wire CLOSED** ✅ (PR #401). **Phase 11.1.E Cultivation Method element affinity wire CLOSED** ✅ (PR #405 backend + PR #408 FE badge). **Phase 11.6.D Talent passive element_resist wire CLOSED** ✅ (PR #409). **Phase 11.6.E Equipment elemental resist runtime CLOSED** ✅ (PR #411). **Phase 11.6.E FE tooltip render CLOSED** ✅ (PR #412). **Phase 11 nâng cao §5 PR1 CLOSED** ✅ (PR #413). **Phase 11 nâng cao §5 PR2 prep CLOSED** ✅ (PR #414). **Phase 11 nâng cao §5 PR2 wire CLOSED** ✅ (PR #415). **Phase 11 nâng cao §5 PR2 smoke negative-path CLOSED** ✅ (PR #416 — 6 mirror negative-path step). **Phase 11 nâng cao §5 PR2 smoke RNG positive-path CLOSED** ✅ (PR #417 — 6 positive-path step admin grant-exp peak seed). **Phase 11 nâng cao §5 PR3 backend prep IN-FLIGHT** (this PR — `GET /character/breakthrough/log` endpoint + service method + tests + smoke extend 31 → 34 step). Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** via #397; Phase 12.2 DungeonTemplate/DungeonRun runtime is next. Admin seed harness đầy đủ 8 endpoint. Smoke scripts **26 module** complete. Detail ở `## 3. Current Phase Status`.
- **Test baseline (post PR #417 merged + breakthrough/log endpoint this PR)**: shared **1276/1276** (unchanged) + api **1813/1813** (1809 baseline + **4 new** `listBreakthroughAttemptLogs` test — character chưa log empty + sort DESC + BigInt cast + limit clamp + cross-user isolation) + web **1034/1034** (unchanged — không touch FE). 5 redis-dependent test (rate-limiter + health controller) cần Redis local — pass khi Redis container up. Smoke scripts **26 module** complete; `smoke:breakthrough` 34/34 OK (31 → 34 step), 2 lần liên tiếp deterministic. Detail ở `## 5. Tests`.
- **Open PR / pending branch**: 1 in-flight backend endpoint PR (this PR — `GET /character/breakthrough/log` + service method + tests + smoke step). PR #417 + #416 + #415 + #414 + #413 + #412 + #411 + #410 + #409 + #408 đã merged. Older docs/audit in-flight (session 5/6 + 5/7) chưa rebase — xem GitHub PR list ở `https://github.com/hoathienmenh-01/xuantoi/pulls`.
...
- **Immediate next task** (3-5 ưu tiên cao nhất theo SESSION PR LIMIT + GOM TRƯỚC KHI TÁCH 4b — Medium PR > Hotfix > Large):
  1. **Phase 11 nâng cao §5 PR3 backend prep this PR land** (in-flight) — `GET /character/breakthrough/log` endpoint + `CharacterService.listBreakthroughAttemptLogs` + 4 service test + smoke extend 31 → 34 step.
  2. **Phase 11 nâng cao §5 PR3 FE — Breakthrough UI history view** — Medium PR, Template B. Cultivation view → "Đột phá" button toggle giữa endpoint cũ/mới (RNG-attempt mode) + chance breakdown tooltip + history list consume `GET /character/breakthrough/log`. E2E spec verify success/fail/Tâm Ma debuff visible.
...
| PR | Title | Type |
|---|---|---|
| (in-flight) | `feat(api): Phase 11 nâng cao §5 PR3 backend prep — GET /character/breakthrough/log endpoint + listBreakthroughAttemptLogs service + 4 test + smoke +3 step` | small backend endpoint |
