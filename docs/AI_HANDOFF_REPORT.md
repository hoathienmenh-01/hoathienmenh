...
- **In-flight**: this PR = E2E spec #22 dungeon-run flow (Hotfix-Small Playwright spec). **Context**: PR #436 đã wire per-encounter loot drop phía BE + FE render — nhưng chưa có E2E spec verify full flow từ UI (start → next → claim) qua thread từ player perspective. Spec #22 bổ sung Playwright golden-path flow nhốt toàn bộ vòng dungeon: register → onboard → navigate `/dungeon-run` → click `dungeon-run-start-son_coc` → next×3 (verify kill log loot span hiển thị) → status COMPLETED → click `dungeon-run-claim` → modal hiển thị reward (linhThach 50 + exp 100 + huyet_chi_dan ×1) → cross-check `/api/character/me` linhThach delta = 50 + `/api/inventory` huyet_chi_dan qty ≥ 1. Spec verify Phase 12.3 invariant: kill log loot span phan biệt qua data-testid pattern `dungeon-run-killed-{i}-loot` chỉ hiển thị sau next() khi `killedEntry.loot.length > 0`. Total vitest unchanged (E2E spec không wire vào vitest); E2E golden-path **22/22 spec** (+1 từ 21). Spec gate `E2E_FULL=1` — mặc định skip trên CI hiện tại (CI chỉ chạy Vitest + AuthView smoke).
- **Test baseline (post #22 spec)**: api **1972** unchanged, shared **1340** unchanged, web **1159** unchanged, total **4471 vitest**. E2E golden-path **22 spec** (+1 từ 21). Smoke scripts **28 module** unchanged. Local verification PASS (typecheck 3/3 + lint 3/3 + Playwright `--list` 23 test parsed clean).
...
| PR | Title | Type |
|---|---|---|
| **this PR** | `test(web): E2E spec #22 dungeon-run flow — son_coc start → next×3 → COMPLETED → claim end-to-end Playwright golden-path, verify Phase 12.3 kill log loot span (dungeon-run-killed-{i}-loot) + claim modal reward (linhThach 50 + exp 100 + huyet_chi_dan) + character/inventory delta cross-check (gate E2E_FULL=1)` | hotfix-small e2e spec, **no BE/FE source change**, **no Prisma migration** |
...
|---|---|---|
| `apps/api` | **1972** | Carryover từ PR #436. |
...
1. **E2E spec #22 dungeon-run flow** **CLOSED** ✅ (this PR). Playwright spec append vào `golden.spec.ts` — son_coc start → next×3 (verify Phase 12.3 kill log loot span `dungeon-run-killed-{i}-loot`) → COMPLETED → claim modal → reward delta cross-check (linhThach +50, inventory huyet_chi_dan qty ≥ 1). Server-authoritative — KHÔNG self-grant. Gate `E2E_FULL=1`.
