...
- **Current `main` commit**: post PR #458 merged (`fix(api,docs): post Phase 13.1.A sect war hardening audit — daily cap window timezone alignment (ICT 00:00 thay vì UTC 00:00) + doc UNIQUE order fix`) + PR #457 (Phase 13.1.A Sect War Core) + PR #456 (post-Phase-13.0 audit pass #5) + Phase 13.0 LiveOps & Retention Suite + PR #451 (Phase 11.10.E Pill→Buff) + PR #450 (M10 Shop daily limit) + ... Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **CLOSED** ✅ (#397–#448). Phase 13.0 LiveOps & Retention Suite **CLOSED** ✅ (post #456 audit pass #5). **Phase 13.1.A Sect War Core / Contribution / Leaderboard / Weekly Reward** **CLOSED** ✅ (PR #457 merged + PR #458 hardening audit). **Phase 13.1.B Sect Missions + Sect Shop + Admin LiveOps Controls** **OPEN** — this PR (#459).
- **In-flight**: this PR (#459) — **Phase 13.1.B Sect Missions + Sect Shop + Admin LiveOps Controls**. Backend: `SectMissionService` (5 mission daily/weekly cadence với target progress, idempotent claim, ledger `SECT_MISSION_CLAIM`); `SectShopService` (5 entries `cong_thien_dan` / `thanh_lam_dan` / `co_thien_dan` / `tinh_huyet_phu` / `tang_son_phu`, contribution spend, daily/weekly per-user caps, NON_STACKABLE_QTY_GT_1 guard, RATE_LIMITED 30 req/60s qua FailoverRateLimiter, atomic CAS race-safe `Character.contribBalance`, ledger `SECT_SHOP_BUY`); `AdminLiveOpsService` (`/admin/liveops` status + `/admin/liveops/event/toggle` upsert `LiveOpsEventOverride` + audit `ADMIN_LIVEOPS_OVERRIDE`, `/admin/sect-war/{status,recalculate}` admin-only). Prisma migration `20260601000000_phase_13_1_b_sect_missions_shop_liveops` (4 table mới: `Character.contribBalance`/`Character.contribLifetime` + `SectMissionClaim` composite UNIQUE `(characterId, missionKey, periodKey)` chống double-claim + `SectShopPurchase` ledger + `LiveOpsEventOverride` upsert by `eventKey`). FE: `/sect-war` 5-tab view (`overview/leaderboard/missions/shop/rewards`), `SectMissionPanel`, `SectShopPanel`, `AdminLiveOpsPanel`, Home/Sect/Admin integration, i18n vi/en parity. Tests: shared catalog drift + 12 SectShopService + 10 AdminLiveOpsService + 12 SectMissionService + 5 SectMissionPanel + 7 SectShopPanel + 4 AdminLiveOpsPanel + 8 SectWarView (tab system migration). KHÔNG đụng SectWar core (read-only).
- **Test baseline (post this PR #459)**: shared **1463** (no change vs PR #457). api **2118** (+40 từ Phase 13.1.B: SectMission service +12, SectMission integration tests carryover, SectShop service +12, AdminLiveOps service +10, plus filtered admin/sect/shop suites). web **1265** (+16 từ 3 panel tests + tab migration fixes). E2E golden-path **22 spec**. Smoke scripts **28 module**.
- **Top priority next session**: (1) Phase 12 Story dialogue branch / story dungeon instance; (2) Daily-login multi-day smoke positive; (3) CSP production verify khi deploy (M7); (4) Admin LiveOps advanced controls (manual force-spawn boss override, season schedule editor); (5) Phase 13.2 Cross-sect seasonal expansion / auction / diplomacy. Detail §6.
...
| 13.1.B | Sect Missions + Sect Shop + Admin LiveOps Controls | **OPEN** (this PR #459) | Server-audited mission ledger + contribution-currency shop với caps/CAS/RATE_LIMITED + admin event toggle + sect-war recalc. Migration `20260601000000_phase_13_1_b_sect_missions_shop_liveops`. |
...
|---|---|---|
| `apps/api` | **2118** | +40 từ Phase 13.1.B: SectMissionService (12) + SectShopService (12) + AdminLiveOpsService (10) + 6 SectMission integration carry. Pre-existing `chat.service.test.ts` rate-limit window test occasionally flakes when run in parallel (2117/2118 trong 1 lần full); pass cô lập (`pnpm --filter @xuantoi/api test -- --run chat.service`). 6 redis-dependent tests (rate-limiter + health) require Redis up. |
| `packages/shared` | **1463** | Không thay đổi vs PR #457 — Phase 13.1.B chỉ thêm 2 catalog `sect-missions.ts` + `sect-shop.ts` (test carryover từ branch). |
| `apps/web` | **1265** | +16 từ Phase 13.1.B: SectMissionPanel (5) + SectShopPanel (7) + AdminLiveOpsPanel (4). Plus +8 SectWarView migration test (tab system route query mock — Phase 13.1.B). |
| **Total** | **4846 vitest** | All green local (Vitest run trên `devin/1778166762-phase-13-1-b-sect-missions-shop-liveops`, PG+Redis up). **GitHub CI runners**: verify khi PR push. |
...
4. **Admin LiveOps advanced controls** (Medium PR) — Phase 13.1.B đã ship minimal (`/admin/liveops` toggle event override + audit + sect-war recalc). Mở rộng: manual force-spawn boss override, season schedule editor, mission/shop catalog hot-swap.

5. **Phase 13.2 Cross-sect seasonal expansion / auction / diplomacy** (Large PR, không gấp) — Phase 13.1.B đã đóng Sect Missions + Sect Shop + Admin LiveOps. Mở rộng cross-sect: seasonal cross-sect war bracket, sect-vs-sect auction, sect diplomacy / alliance / declaration of war.
