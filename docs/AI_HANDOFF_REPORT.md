...
- **Current `main` commit**: post PR #445 merged (`fix(api,cultivation): tick CAS guard + 2 race backstop test`) + PR #444 (Realtime ban-during-connection + kickUser) + PR #443 (Inventory `revoke()` race fix) + PR #442 (Phase 12.5 dungeon balance tuning) + PR #441 (docs handoff sync post #440) + PR #440 (Phase 12 Story discoverability QuestView dungeon hint) + PR #439 (Story Foundation Late-game encounter wire) + PR #438 (Phase 12.4 per-monster MonsterDef.lootTable polish) + PR #437 (DungeonRun listForUser activeRun fallback + E2E spec #22) + PR #436 (Phase 12.3 DungeonRun per-encounter loot wire). Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), Phase 12 Story PR-1→PR-6 + Foundation Extension + Late-game wire all **CLOSED** ✅ (#425–#433), **Phase 12.2.B/C/12.3** **CLOSED** ✅ (#434–#436), **E2E spec #22 + listForUser fallback** **CLOSED** ✅ (#437), **Phase 12.4 per-monster MonsterDef.lootTable polish** **CLOSED** ✅ (#438), **Story Foundation Late-game encounter wire** **CLOSED** ✅ (#439), **Phase 12 Story discoverability — QuestView dungeon hint cho kill+monster step** **CLOSED** ✅ (#440), **docs handoff sync post #440** **CLOSED** ✅ (#441), **Phase 12.5 dungeon balance tuning** **CLOSED** ✅ (#442), **Concurrency phase 2 — Inventory `revoke()` race fix** **CLOSED** ✅ (#443), **Concurrency phase 2 — Realtime ban-during-connection hardening + kickUser** **CLOSED** ✅ (#444), **Concurrency phase 2 — Cultivation tick CAS guard** **CLOSED** ✅ (#445), **Concurrency phase 2 — Chat Redis failover (FailoverRateLimiter wrap)** **CLOSED** ✅ (this PR). Next mở: Concurrency phase 2 remainder (Boss spawn cron auto race).
- **In-flight**: this PR (Concurrency phase 2 — Chat Redis failover branch via FailoverRateLimiter wrapper).
- **Test baseline (post this PR)**: api **1991** (+7: 6 unit FailoverRateLimiter + 1 integration real Redis disconnect), shared **1370**, web **1163**, total **4524 vitest**. E2E golden-path **22 spec**. Smoke scripts **28 module**.
- **Top priority next session**: (1) Boss spawn cron auto race — Concurrency phase 2 final remainder (~50-100 LOC); (2) Shop daily limit + rate-limit per user (M10) Medium PR; (3) CSP production verify khi deploy (M7); (4) Phase 12 Story dialogue branch / DungeonTemplate story-instance (optional). Detail §6.
...
| PR | Title | Type |
|---|---|---|
| this PR | `feat(api,common,chat): FailoverRateLimiter Redis-down failover + 7 test — ChatService.send() trong production wire `RedisSlidingWindowRateLimiter` ở chat.module factory; nếu Redis sống lúc bind nhưng chết runtime (pod restart, network partition, NOAUTH after rotation), `pipeline.exec()` throw → ChatService propagate 500 → user mất quyền chat. Thêm `FailoverRateLimiter(primary, fallback, logger)` wrapper trong rate-limiter.ts: `try primary.check; catch → log warn (1 lần) + fallback.check`. Wire chat.module factory wrap Redis limiter với in-memory fallback; Redis recover → tự quay lại Redis path. Trade-off: spam window degrade `8 msg/30s cụm → 8 msg/30s instance` trong khoảng Redis down, đổi lại uptime. +6 unit test (primary OK / throw / log warn 1x / recover / fallback enforce limit / resetWarning) + 1 integration test real Redis client.disconnect() runtime.` | small-medium api fix + tests, no Prisma migration, no UI change |
...
|---|---|---|
| `apps/api` | **1991** | +7 từ Concurrency phase 2 Chat Redis failover (`rate-limiter.test`: 6 unit FailoverRateLimiter primary OK / throw → fallback / warn 1x no spam / recovery / fallback in-memory limit enforced / resetWarning post-probe + 1 integration real Redis disconnect). Carryover #445. |
| `packages/shared` | **1370** | Carryover từ #442 (Phase 12.5 shared catalog tuning). |
| `apps/web` | **1163** | Carryover từ #440 (no UI change). |
| **Total** | **4524 vitest** | All green local. **GitHub CI runners**: verify khi PR push. |
...
1. **Concurrency tests phase 2 remainder** — Small-Medium PR, Template B per scenario. Inventory `use()` race FIXED via PR #422; Inventory `revoke()` race FIXED via PR #443; Realtime ban-during-connection FIXED via PR #444; Cultivation tick CAS guard FIXED via PR #445; **Chat Redis failover branch FIXED via this PR** (FailoverRateLimiter wrapper try-primary-catch-fallback, log warn 1x to avoid spam, in-memory degrade per-instance trong khoảng Redis down, auto-recover khi Redis up). Còn lại: Boss spawn cron auto race (~50-100 LOC test).
