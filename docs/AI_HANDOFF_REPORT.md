...
- **Current `main` commit**: post PR #451 merged (`feat(api,shared): Phase 11.10.E Title/Buff gameplay reward hooks — Pill→Buff wire`) + PR #450 (M10 Shop daily limit + per-user rate limit) + PR #449 (Phase 11.8.D Buff HUD + 11.9.C Title catalog/equip FE wire) + PR #448 (Phase 12.6 Boss-by-region auto-spawn) + PR #447 (spawn cron auto race + partial unique index) + PR #446 (FailoverRateLimiter Redis-down) + PR #445 (Cultivation tick CAS guard) + PR #444 (Realtime ban-during-connection) + PR #443 (Inventory `revoke()` race fix) + PR #442 (Phase 12.5 dungeon balance tuning). Story design source live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 **CLOSED** ✅. Phase 11 Progression Depth **COMPLETE** ✅. Phase 11 nâng cao **CLOSED** ✅. Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** (#397), Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ (#421), Phase 12 Story PR-1→PR-6 + Foundation Extension + Late-game wire all **CLOSED** ✅ (#425–#433), **Phase 12.2.B/C/12.3** **CLOSED** ✅ (#434–#436), **E2E spec #22 + listForUser fallback** **CLOSED** ✅ (#437), **Phase 12.4** **CLOSED** ✅ (#438), **Phase 12 Story discoverability** **CLOSED** ✅ (#439–#441), **Phase 12.5 dungeon balance tuning** **CLOSED** ✅ (#442), **Concurrency phase 2** **CLOSED** ✅ (#422 + #443–#447), **Phase 12.6 Boss-by-region auto-spawn** **CLOSED** ✅ (#448), **Phase 11.8.D Buff HUD + 11.9.C Title catalog/equip FE wire** **CLOSED** ✅ (#449), **M10 Shop daily limit + rate-limit per user** **CLOSED** ✅ (#450), **Phase 11.10.E Pill→Buff wire** **CLOSED** ✅ (#451), **Phase 13.0 LiveOps & Retention Suite** **OPEN** — this PR (LiveOpsEventDef catalog + scheduled boss heartbeat + reward title/buff hooks + `/liveops/today` API + Today Activity panel + BossView schedule UI + upcoming-boss notice).
- **In-flight**: this PR — **Phase 13.0 LiveOps & Retention Suite**. Shared: `LiveOpsEventDef` catalog (5 events: 3 daily boss 12:00/19:00/22:00 ICT + Huết Nguyệt weekly Sat 21:00 ICT + 2 buff event), helpers `liveOpsEventsForToday()`/`activeLiveOpsEvents()`/`nextLiveOpsEvent()`/`bossScheduleForToday()`/`liveOpsEventForBossSpawn()`. Catalog title +1 (`event_huyet_nguyet_2026`). API: BossService heartbeat đọc schedule, spawn boss khi tới slot với dedup `(regionKey,bossKey,spawnedAt>=slotStart)` — KHông schema field. distributeRewards() add reward hooks (participation title `achievement_first_boss` + event title `event_huyet_nguyet_2026` + top-1 buff `event_double_drop`) defensive try-catch. Mới: `LiveOpsModule` + `GET /liveops/today` (pure compute, no auth). FE: `LiveOpsTodayPanel`/`BossSchedulePanel`/`LiveOpsNotice` + i18n vi/en cho 5 event + 4 boss + 4 region; HomeView mount panel/notice; BossView mount schedule panel. Notice anti-spam per slot via sessionStorage. KHÔNG Prisma migration.
- **Test baseline (post this PR)**: shared **+45** (liveops calendar/helpers/event-for-spawn). api **+22** (boss schedule **+9**, reward hooks **+7**, liveops service **+7**). web **+15** (LiveOpsTodayPanel **+6**, BossSchedulePanel **+3**, LiveOpsNotice **+6**). cumulative **shared 1424 / api 2061 / web 1231**. E2E golden-path **22 spec**. Smoke scripts **28 module**.
- **Top priority next session**: (1) Phase 12 Story dialogue branch / story dungeon instance; (2) Daily-login multi-day smoke positive; (3) CSP production verify khi deploy (M7); (4) Admin LiveOps controls (CMS toggle/event override); (5) Phase 13.1 Sect War / weekly competition planning. Detail §6.
...
| PR | Title | Type |
|---|---|---|
| this PR | `feat(api,web,shared): Phase 13.0 LiveOps & Retention Suite — shared `LiveOpsEventDef` static catalog (5 events: 3 daily boss `boss_daily_noon_hoa_diem_son`/`boss_daily_evening_kim_son_mach`/`boss_daily_night_hoang_tho_huyet` 12:00/19:00/22:00 ICT + `event_huyet_nguyet_weekend` Sat 21:00 ICT cho Cửu La Thiên Đế + 2 buff event `event_daily_exp_rush`/`event_weekly_dungeon_double_drop`); helpers `liveOpsEventsForToday()`/`activeLiveOpsEvents()`/`nextLiveOpsEvent()`/`bossScheduleForToday()`/`liveOpsEventForBossSpawn()` (deterministic, tz `Asia/Ho_Chi_Minh` mặc định reuse `MISSION_RESET_TZ`); +1 title `event_huyet_nguyet_2026` (epic, source=event). API: `BossService.tickHeartbeat()` extension — mỗi region check schedule, gọi `prisma.worldBoss.findFirst({ regionKey, bossKey, spawnedAt: {gte: slotStart} })` dedup → spawn nếu chưa có (idempotent qua slot dedup, KHÔNG schema field, an toàn parallel heartbeat); `distributeRewards()` reward hooks (defensive try-catch trong tx) — mọi participant unlock `achievement_first_boss`, top-1 apply `event_double_drop` 1h buff, nếu spawn từ `event_huyet_nguyet_weekend` slot → unlock `event_huyet_nguyet_2026`. Mới `LiveOpsModule` + `GET /liveops/today` (no auth, pure compute < 1ms): trả `{nowIso, timezone, todayEvents, activeEvents, nextEvent, bossSchedule, suggestedActivities}`. FE: `LiveOpsTodayPanel` (HomeView retention hub: suggested CTA + active events + boss schedule + 3 CTA button); `BossSchedulePanel` (BossView "Lịch Boss hôm nay" — slot list status badge + countdown); `LiveOpsNotice` (renderless, poll 60s, push toast warning khi upcoming boss ≤ 15 min, anti-spam per slot via sessionStorage); i18n vi/en cho `liveopsToday.*` + `liveops.event/boss/region.*`. KHÔNG Prisma migration. Tests: shared **+45** liveops; api **+9** boss schedule + **+7** reward hooks + **+7** liveops service; web **+6** LiveOpsTodayPanel + **+3** BossSchedulePanel + **+6** LiveOpsNotice. Cumulative shared 1424 / api 2061 / web 1231. Verification: typecheck shared/api/web xanh; pnpm build xanh.` | feat |
...
5. **Phase 13.1 Sect War / weekly competition planning** (Large PR design) — sect-vs-sect leaderboard reward, scheduled cuối tuần (Sun 20:00 ICT?), wire vào LiveOps catalog.
