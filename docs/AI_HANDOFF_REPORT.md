...
- **In-flight**: this PR = Phase 12 Story Runtime MVP (Quest UI: `QuestView.vue` + Pinia store `useQuestStore` + 3 typed API wrappers + router `/quests` + nav AppShell + i18n vi/en parity; consume `GET /quests/me` + `POST /quests/accept` + `POST /quests/claim` PR-2/3 sẵn có, KHÔNG endpoint mới, KHÔNG Prisma migration). Pair với Story Foundation Extension PR #429 extending catalog 5 cảnh giới đầu (Kim Đan + Nguyên Anh).
- **Test baseline (post Phase 12 Story Runtime MVP)**: api **1907**, shared **1328** (PR #429 catalog extension), web **1126** (+25 from `stores/__tests__/quest.test.ts` 12 + `views/__tests__/QuestView.test.ts` 13), total **4361 vitest**. E2E golden-path **20/20 spec**. Smoke scripts **27 module**. CI expected GREEN.
...
| [pending #429](https://github.com/hoathienmenh-01/xuantoi/pull/429) | `feat(shared): Phase 12 Story Foundation Extension — Kim Đan + Nguyên Anh catalog (+10 quest + 1 NPC Huyết La Sát + 5 dialogue line + integrity test)` | small shared catalog + tests, KHÔNG runtime change |
