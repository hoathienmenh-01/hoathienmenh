...
- **Current `main` commit**: post PR #423 merged (`docs(story): add Tu Tien Lo story bible and Phase 12 progress tracker`) + PR #422 (Inventory concurrency hardening — `InventoryService.use()` atomic decrement fix + 5 race regression test) + PR #421 (Phase 12.2.A `DungeonDef.dailyLimit` server-side enforcement). Story Bible DOCX archived + markdown bible + Phase 12 progress tracker live tại [`docs/story/`](./story/).
- **Current phase**: Phase 10 Content scale **CLOSED** ✅ Phase 11 Progression Depth **COMPLETE** ✅ (catalog 11/11 + runtime 10/10 + UI tracks merged via #394). Phase 11 nâng cao **CLOSED** ✅ (§2/§3/§5/§6 + Phase 11.6.C/D/E + Phase 11.1.E). Phase 12 World Map & Dungeon **OPEN** — Phase 12.1 catalog **CLOSED** via #397, Phase 12.2.A `DungeonDef.dailyLimit` **CLOSED** ✅ via #421. Story design source archived via #423 (runtime story/quest/NPC chưa implemented).
- **In-flight**: none (this PR = docs(ai) compact handoff + task-based docs nav, docs-only).
- **Test baseline (post #423 merged)**: api **1825**, shared **1276**, web **1082**, total **4183 vitest**. E2E golden-path **20/20 spec** (gated `E2E_FULL=1`; CI `e2e-smoke` chạy spec #1 mỗi PR). Smoke scripts **25 module** (~15 step avg, 8 module có positive-path coverage). 1 flaky chat rate-limit timing test passes on retry. CI 5/5 GREEN.
- **Top priority next session**: (1) Phase 12.2.B DungeonTemplate + DungeonRun runtime (Medium-Large, **Prisma migration risk**); (2) Phase 12 PR-1 Story/NPC/Quest catalog foundation (Small-Medium, no Prisma); (3) smoke:daily-login multi-day positive (Small); (4) Concurrency tests phase 2 backlog. Detail §6.
...
| [#415](https://github.com/hoathienmenh-01/xuantoi/pull/415) | `feat(api): Phase 11 §5 PR2 wire — CharacterService.attemptBreakthrough + POST /character/breakthrough/attempt + cultivationRateMul wire` | medium BE wire + tests |
| [#414](https://github.com/hoathienmenh-01/xuantoi/pull/414) | `feat(shared,api): Phase 11 §5 PR2 prep — tam_ma_light buff catalog + cultivation_rate_mul effect + evaluateBreakthroughOutcome + BreakthroughAttemptLog Prisma model` | medium shared+Prisma prep + tests |

**Phase summary tables (PR #1 → #396)**: tách sang [`ARCHIVE_HANDOFF.md`](./ARCHIVE_HANDOFF.md) § Phase Summary Migrated 2026-05-05.
...
| 12 | World Map & Dungeon | **OPEN** | 12.1 catalog CLOSED (#397). 12.2.A `dailyLimit` enforcement CLOSED (#421). 12.2.B DungeonTemplate runtime is next. Story design source archived (#423); runtime quest/NPC chưa start. |
...
### Vitest baseline (post PR #423 merged)

| Workspace | Test count | Notes |
|---|---|---|
| `apps/api` | **1825** | +5 from PR #422 inventory concurrency regression test. 1 flaky pre-existing chat rate-limit timing test (50ms sliding window) passes on retry. |
| `packages/shared` | **1276** | Carryover từ PR #414. |
| `apps/web` | **1082** | Carryover từ PR #419. |
| **Total** | **4183 vitest** | All green trên main. |
...
2. **Phase 12 Story chain** — design source ở [`docs/story/TU_TIEN_LO_STORY_BIBLE.md`](./story/TU_TIEN_LO_STORY_BIBLE.md) + [`docs/story/PHASE12_STORY_PROGRESS.md`](./story/PHASE12_STORY_PROGRESS.md). 5-PR roadmap:
   - **PR-1** — Story/NPC/Quest catalog foundation (`packages/shared/src/quests.ts`, `npcs.ts`, `dialogues.ts` cho Phàm Nhân + Luyện Khí + Trúc Cơ = 15 quest + 4 NPC). Small-Medium, no Prisma.
   - **PR-2** — Quest runtime persistence (Prisma `QuestProgress` + `Character.storyChapter` migration + `QuestService.list/accept/progress/complete` + smoke). Medium, **Prisma risk**.
   - **PR-3** — Quest claim / reward idempotency (`QuestService.claim` qua `RewardLedger` + idempotency `(characterId, QUEST_CLAIM, questId)`). Small.
