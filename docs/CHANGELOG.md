...
### Added — Phase 13.2.C Sect Season History + Hall of Fame (this PR)
...
- **`SectSeasonHistoryService`** (`apps/api/src/modules/sect-season/sect-season-history.service.ts`):
  - **`snapshotSeason(seasonKey)` IDEMPOTENT** — check `findUnique({ seasonKey })` trước → return existing nếu có (no double snapshot, no Prisma `P2002` race). Nếu chưa → derive sects + topMembers từ `SectWarContribution` qua `sectSeasonWeekKeys(season)` (logic giống leaderboard live), rồi 1 transaction `prisma.sectSeasonSnapshot.create({ data: { ..., sects: { create: [...] }, topMembers: { create: [...] } } })` nested write atomic. KHÔNG có endpoint trigger từ player — gọi qua test/dev script hoặc Phase 13.2.B+ settlement runtime sau này. Phase 13.2.C **chưa wire cron auto-snapshot**.
  - **`listHistory()`** — `findMany order finalizedAt desc` map snapshot header → `SectSeasonHistorySummary[]`.
  - **`getHistory(seasonKey)`** — full snapshot detail; map sects/topMembers `rank asc`.
  - **`getHallOfFame()`** — load tất cả snapshot + nested sects/topMembers, fold qua `Map<sectId, SectHallOfFameSectEntry>` + `Map<characterId, SectHallOfFameMemberEntry>` aggregate `championships`/`mvps`/`podiums`/`appearances`/`bestRank`/`totalPoints`/`latestSeasonKey`/`latestSectName` rồi sort theo ordering ở trên. `championships` += 1 khi `rank=1` của sect; `mvps` += 1 khi `rank=1` của member; `podiums` += 1 khi `rank ∈ {1,2,3}`.
- **FE mới**:
  - **`SectSeasonHistoryPanel.vue`** (`apps/web/src/components/`) — list view (mùa cards với champion/MVP summary inline + nút "Xem chi tiết") + detail view (full leaderboard 2-table sects + top 10 members) + Hall of Fame section (sects table + members table) + back button + error/empty states. Dùng `getSectSeasonHistory()` + `getSectSeasonHistoryDetail(seasonKey)` + `getSectSeasonHallOfFame()` parallel ở mount via `Promise.all`.
  - **`SectWarView.vue`** thêm tab `hallOfFame` — `SectWarTab` union extend `overview/leaderboard/missions/shop/rewards/season/hallOfFame`. Click tab Hall of Fame → mount `SectSeasonHistoryPanel` (no props).
  - **API client** (`apps/web/src/api/sectSeason.ts`): `getSectSeasonHistory()`, `getSectSeasonHistoryDetail(seasonKey)`, `getSectSeasonHallOfFame()` — envelope unwrap pattern.
- **i18n vi/en parity** (`sectWar.tab.{season,hallOfFame}` + `sectSeason.history.*` + `sectSeason.hallOfFame.*`): tab labels, history list/detail empty + loading + error (`SNAPSHOT_NOT_FOUND`/`SEASON_NOT_FOUND`/`UNKNOWN`), champion/MVP labels + fallback (no champion/no MVP), Hall of Fame columns (`rank/sect/member/championships/mvps/podiums/appearances/bestRank/totalPoints/latest/latestSect`).
- **Tests mới**:
  - **API +9 case** `apps/api/src/modules/sect-season/sect-season-history.service.test.ts`: snapshot empty season trả 0 totals + null champion/mvp; snapshot real season derive sects + topMembers + denormalized champion/mvp; snapshot idempotent (call 2× same season → 1 row only, không P2002); listHistory empty + ordering; getHistory by key + 404 SNAPSHOT_NOT_FOUND + SEASON_NOT_FOUND; getHallOfFame aggregate ordering (championships desc → mvps desc → podiums desc → totalPoints desc → sectId asc / characterId asc); getHallOfFame multi-season cumulative.
  - **Web +7 case** `apps/web/src/components/__tests__/SectSeasonHistoryPanel.test.ts`: render list view 2 mùa champion/MVP summary; empty state list + Hall of Fame; Hall of Fame sects + members ordering theo server response; viewDetail click → fetch detail → render full leaderboard + top members + member without sect fallback `(không Tông)`; back button quay lại list; error state list `UNKNOWN`; detail error `SNAPSHOT_NOT_FOUND` fallback i18n.
- **Risk / rollback**: thấp-trung bình. Migration mới (3 bảng) **append-only**, KHÔNG đụng schema hiện có (`SectWarContribution`/`SectWarRewardClaim`/`Character`/`Sect`/`SectSeason*` Phase 13.2.A). API endpoints mới read-only — KHÔNG grant reward, KHÔNG mutate state, KHÔNG đụng claim flow Phase 13.1.A. Snapshot service idempotent qua `seasonKey` UNIQUE — race-safe nếu cron auto-snapshot wire sau. Rollback = revert PR + drop 3 bảng (an toàn vì không có FK external trỏ vào).
- **Out of scope** (theo scope user request): KHÔNG implement season settlement automation phức tạp (cron auto-snapshot deferred Phase 13.2.B+); KHÔNG grant reward / title award khi snapshot (deferred Phase 13.2.B+ reward claim runtime); KHÔNG editor CMS cho season catalog; KHÔNG per-season milestone customization; KHÔNG đụng PvP realtime / auction / diplomacy / alliance.
- **Verification**: shared typecheck ✅ / api typecheck ✅ + `--run sect-season` 31 PASS ✅ (16 cũ Phase 13.2.A + 9 mới Phase 13.2.C + 6 controller smoke giữ nguyên) + `--run sect-war` 15 PASS ✅ (zero regression) / web typecheck ✅ + `--run SectSeason` 14 PASS ✅ (7 cũ Phase 13.2.A + 7 mới Phase 13.2.C) / pnpm build pending ⏳ (chạy trước create PR).
- **Next roadmap**: (1) Phase 13.2.B Sect Season Reward Claim — UI claim milestone reward + idempotent grant qua ledger + audit; (2) Phase 13.2.C-runtime — cron auto-snapshot mùa khi `currentSectSeason(now)` chuyển từ `season_X → null` hoặc `→ season_Y`; (3) admin manual trigger snapshot endpoint cho dev/staging; (4) badge/title award cho champion sect + MVP cá nhân (parity Phase 11.9.C title catalog).
