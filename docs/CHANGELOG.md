...
### Added — Phase 26.3 Cultivation Method V2 / Công Pháp Progression & Fragment System

- Added shared catalog `packages/shared/src/cultivation-methods-v2.ts` (1735 lines) with **36 cultivation methods** across **9 grade tiers** (PHAM → HUYEN → DIA → THIEN → TIEN → THAN → THANH → CHUAN_THANH → CHI_TON), **7 categories** (QI / BODY / HYBRID / ELEMENTAL / SECT / FORBIDDEN / SPECIAL), and **5 equip slots** (QI_MAIN / BODY_MAIN / SUPPORT / SECT / SPECIAL). Each `CultivationMethodV2Def` exposes `tier`, `unlockRealmOrder`, `unlockBodyRealmOrder`, `requiredSect`, `maxLevel`, `maxStar`, `fragmentItemKey`, `fragmentsRequired`, `fragmentsPerStar`, `upgradeMaterials`, `breakthroughMaterials`, `baseStats`, `perLevelStats`, `perStarStats`, `passiveEffects`, `sourceHint`, `primarySlot`, `allowedSlots`, `tradeable`, `bindOnUnlock`, and `unlockLinhThachCost`.
- Added helpers: `methodUpgradeLinhThachCost` / `methodUpgradeExpCost` (monotonic per tier baseline), `computeMethodStatBonus` (level + star scaling), `aggregateEquippedMethods` (sums up to 5 equipped methods, clamps via `METHOD_BONUS_CAPS`), `methodGradeForTier`, `methodElementToElementKey`, `methodFragmentItemKey`, plus gating helpers `canEquipMethod` / `canUpgradeMethod` / `canStarUpMethod` and filter helper `filterMethods`. `METHOD_BONUS_CAPS` ensures bounded stat contribution per surface (qiExpPercent / bodyExpPercent / hpMaxPercent / mpMaxPercent / atkPercent / defPercent / spiritPercent / staminaMaxPercent / bossDamageReduction / elementalAtkBonus / tribulationSupport).
- Added auto-generated `method_fragment_<methodKey>` items wired into Drop Economy V2 — `materialCategory='METHOD_FRAGMENT'`, `materialTier` matches method tier, sourceHint propagated from method.
- Added additive Prisma migration: extends `CharacterCultivationMethod` with `level` (Int), `star` (Int), `methodExp` (BigInt), `equippedSlot` (nullable enum), `updatedAt` (DateTime). New `MethodUpgradeLog` table audits unlock / equip / unequip / upgrade / star-up with `op`, `methodKey`, `linhThachSpent`, `fragmentsSpent`, materials JSON, `levelBefore`/`levelAfter`/`starBefore`/`starAfter` snapshot.
- Added `CultivationMethodV2Service` (`apps/api/src/modules/character/cultivation-method-v2.service.ts`, 703 lines) with server-authoritative methods `grantStarterV2IfMissing`, `getV2State`, `unlock`, `equipV2`, `unequipV2`, `upgrade`, `starUp`, `getEquippedSnapshot(Tx)`. All write operations run inside a single `$transaction` and are idempotent. Validation gates: realm/body-realm order, sect lock, fragment + linh thạch stock, slot occupancy, slot allow-list, level/star caps. Equipping QI_MAIN starter mirrors to legacy `Character.equippedCultivationMethodKey` for backward compatibility.
- Added 5 character endpoints: `GET /character/cultivation-methods-v2`, `POST /character/cultivation-methods-v2/unlock`, `POST /character/cultivation-methods-v2/equip`, `POST /character/cultivation-methods-v2/unequip`, `POST /character/cultivation-methods-v2/upgrade`, `POST /character/cultivation-methods-v2/star-up`. All inputs are Zod-validated; `CultivationMethodV2Error` codes are mapped to HTTP status (400 for client input errors, 404 for missing method, 409 for slot conflict, 403 for realm/sect lock).
- Added web `/cultivation-method-v2` view (`apps/web/src/views/CultivationMethodV2View.vue` + `cultivationMethodV2` Pinia store + `cultivationMethodV2.ts` API client). UI surfaces: slot summary (5 slot cards with equipped name + level + star + unequip button), aggregated bonus chips (qiExp/bodyExp/hpMax/mpMax/atk/def/spirit/stamina/bossDmgReduction/elementalAtk/tribulationSupport — only non-zero rendered), filter UI (category, slot, unlocked-only, clear button), and per-method card with name + tier + grade + category badges, fragment progress, level/star, unlock cost or upgrade cost, source hints, and unlock/equip/upgrade/star-up actions. Loading / error / empty / list states all covered. Toast notifications surface success/error i18n. Race-protected via `inFlight` Set in store.
- Added i18n parity in `vi.json` + `en.json`: title/subtitle, slot names, grade names (9), category names (7), filter labels, summary labels, stat labels (11 percent-formatted), field labels (level/star/fragments/unlock cost/upgrade cost/tier/source/equipped at), action buttons (unlock/equip/upgrade/starUp + busy state), success messages, and 25+ error codes (METHOD_NOT_FOUND, METHOD_DISABLED, REALM_TOO_LOW, BODY_REALM_TOO_LOW, SECT_LOCK, INSUFFICIENT_FRAGMENTS, INSUFFICIENT_LINH_THACH, METHOD_ALREADY_UNLOCKED, METHOD_NOT_UNLOCKED, SLOT_OCCUPIED, SLOT_NOT_ALLOWED, MAX_LEVEL_REACHED, MAX_STAR_REACHED, INSUFFICIENT_MATERIALS, BREAKTHROUGH_MATERIALS_MISSING, UNKNOWN). Added 6 `common.apiFallback.cultivationMethodV2*` entries for endpoint error fallback.
- Added shell nav link "訣 Công Pháp V2" → `/cultivation-method-v2`.
- Added shared tests (`cultivation-methods-v2.test.ts`, 47 tests) covering catalog invariants, fragment ↔ items.ts cross-check, cost monotonicity, stat composition, cap enforcement, equip/upgrade/star-up gating, element mapping, filter helpers.
- Added API integration tests (`cultivation-method-v2.service.test.ts`, 19 tests) covering grantStarterV2IfMissing idempotency, getV2State catalog hydration, unlock realm/fragment/linh thạch gating + happy path + duplicate-unlock guard, equipV2 unlock check + QI_MAIN legacy mirror + slot allow-list violation, unequipV2 slot clear + legacy field reset, upgrade level progression + materials gate, starUp fragment consume, getEquippedSnapshot 2-slot wire.

#### Balance / Economy
- Method bonuses go through `METHOD_BONUS_CAPS` and only contribute to safe surfaces (cultivation EXP, max stats, attack/defense percent, stamina, boss damage reduction, elemental attack, tribulation support). No direct linhThach/tienNgoc grants from method actions.
- Anti-P2W invariant: tier 8-9 methods (THANH / CHUAN_THANH / CHI_TON) have no real-money source. Fragment drops scale with Drop Economy V2 tier rules and are gated by realm.
- Realm gating: `unlockRealmOrder` enforced server-side; `unlockBodyRealmOrder` separately gated for BODY/HYBRID methods. Sect-locked methods only unlock when character is in the matching sect (`thanh_van`, `huyen_thuy`, `tu_la`).
- Star-up cost: each star tier consumes `fragmentsPerStar` from inventory (auto-derived item key `method_fragment_<methodKey>`). Method exp ledger logs `MethodUpgradeLog` for every mutation.
...
- **Prisma migration `20261020000000_phase_20_1_party_dungeon_coop`** additive: enum `PartyDungeonRoomStatus` (`LOBBY`/`READY_CHECK`/`STARTED`/`COMPLETED`/`FAILED`/`CANCELED`), `PartyDungeonRunResult` (`CLEAR`/`FAIL`/`CANCELED`), `PartyDungeonRewardClaimStatus` (`PENDING`/`CLAIMED`) + model `PartyDungeonRoom`(`id`, `partyId`, `leaderUserId`, `dungeonKey`, `status=LOBBY`, `minMembers=2`, `maxMembers=5`, `createdAt`, `startedAt?`, `finishedAt?`, `canceledAt?`, `currentRunId?` + index `(partyId,status)` cho active-room lookup), `PartyDungeonParticipant`(`id`, `roomId`, `userId`, `characterId?`, `readyAt?`, `joinedAt`, `leftAt?`, `resultStatus?` + unique `(roomId,userId,leftAt)` chống double-join), `PartyDungeonRun`(`id`, `roomId`, `partyId`, `dungeonKey`, `result`, `startedAt`, `finishedAt?`, `combatSummaryJson?`, `rewardSummaryJson?`), `PartyDungeonRewardClaim`(`id`, `runId`, `userId`, `characterId`, `status=PENDING`, `rewardJson`, `claimedAt?`, `createdAt` + unique `(runId,userId)` chống duplicate). Soft-ref pattern (no FK).
- **Shared `packages/shared/src/coop-dungeon.ts`**: enum + DTO (`PartyDungeonRoomDto`/`PartyDungeonParticipantDto`/`PartyDungeonRunDto`/`PartyDungeonRewardClaimDto`/`PartyDungeonRewardPreview`) + response shapes (`MyPartyDungeonRoomResponse`/`PartyDungeonRunDetailResponse`) + `COOP_DUNGEON_LIMITS` (`minMembers=2`, `maxMembers=5`, `maxActiveRoomPerParty=1`, `readyTimeoutSec=300`). Helper `canStartPartyDungeon` server-authoritative gate (catalog miss / not-leader / not-enough-members / not-all-ready). WS payload types `PartyDungeonRoomUpdatedBroadcastPayload`/`PartyDungeonReadyUpdatedBroadcastPayload`/`PartyDungeonStartedBroadcastPayload`/`PartyDungeonCompletedBroadcastPayload`/`PartyDungeonRewardAvailableBroadcastPayload`.
- **Shared `security-rate-limit`**: 4 policy mới group `PARTY` — `PARTY_DUNGEON_CREATE` (20/60min/user block 30p), `PARTY_DUNGEON_READY` (120/60s/user block 5p), `PARTY_DUNGEON_START` (30/60s/user block 5p), `PARTY_DUNGEON_CLAIM` (60/60s/user block 5p). `scope='USER'` `sensitive=true` `severity=MEDIUM`.
- **Shared `ws-events`**: 5 type — `party-dungeon:room-updated`, `party-dungeon:ready-updated`, `party-dungeon:started`, `party-dungeon:completed`, `party-dungeon:reward-available`.
- **Backend `apps/api/src/modules/party-dungeon/`** (9 service method, 25 integration test pass real Postgres): `getMyRoom`/`getRunDetail`/`createRoom`/`joinFromParty`/`setReady`/`cancelReady`/`startRun`/`cancelRoom`/`claimReward`. `startRun` auto-resolve inline trong `prisma.$transaction` → tạo `PartyDungeonRun` + clone `DungeonDef.runReward` cho mỗi participant thành `PartyDungeonRewardClaim` PENDING. `claimReward` atomic CAS `PENDING→CLAIMED` + grant currency/items/exp qua `CurrencyService.applyTx`/`InventoryService.grantTx`/`tx.character.update` (reason `PARTY_DUNGEON_REWARD`, không bypass ledger).
- **REST API** `PartyDungeonController` 9 endpoint cookie-auth dưới prefix `/party/dungeon`: `GET /room` + `/runs/:id` + `POST /rooms` (`PARTY_DUNGEON_CREATE`) + `/join|ready|unready` (`PARTY_DUNGEON_READY`) + `/start|cancel` (`PARTY_DUNGEON_START`) + `/runs/:id/claim-reward` (`PARTY_DUNGEON_CLAIM`).
- **Ledger reason** mở rộng: `LedgerReason` (`CurrencyService`) + `ItemLedgerReason` (`InventoryService`) add `'PARTY_DUNGEON_REWARD'`.
- **FE `apps/web/src/api/partyDungeon.ts`**: 9 fn Envelope wrap.
- **FE `apps/web/src/components/PartyDungeonPanel.vue`** (5 test pass): empty state + create form (dungeon dropdown), lobby/ready-check state với participant list + ready toggle (caller-only) + leader actions (start gated qua `canStartPartyDungeon` shared helper + cancel với `ConfirmModal` teleport-to-body), COMPLETED state với reward preview + claim button (PENDING) hoặc "đã nhận" (CLAIMED), failed/canceled summary. Realtime subscribe 5 event `party-dungeon:*` để refresh inline.
- **FE integration `apps/web/src/views/SocialView.vue`**: thêm tab thứ 5 'partyDungeon' alongside friends/private/group/party.
- **i18n vi/en parity 100%**: ~80 leaf key dưới `partyDungeon.*` (title/subtitle, empty, room summary, 6 status, participants role + ready badge, 6 action, 2 confirm modal, reward preview, 5 toast, 17 error code).

#### Security — Phase 20.1

- 1 active room / party invariant — service guard + Prisma index `(partyId,status)`.
- Leader-only mutations (`createRoom`/`startRun`/`cancelRoom`) — non-leader → `NOT_PARTY_LEADER` (403).
- Caller-scoped participant mutations (`join`/`ready`/`unready`) — không cho inject `userId` khác.
- Dungeon catalog whitelist qua `DUNGEONS` shared — `INVALID_DUNGEON` (400).
- Start gate qua `canStartPartyDungeon` shared (catalog miss / not-leader / not-enough-members / not-all-ready).
- Reward claim race-safe — CAS `WHERE status='PENDING'` + transaction grant ledger.
- Run detail authorization — non-participant fall vào `NOT_PARTY_MEMBER` (KHÔNG leak `RUN_NOT_FOUND`).
- WS event fanout chỉ tới participants (1 recipient cho `reward-available`).
- Audit log meta (`roomId`, `partyId`, `runId?`, `callerUserId`), KHÔNG log reward amount detail.

#### Test coverage — Phase 20.1

- Shared (`coop-dungeon.test.ts`): enum + DTO + `COOP_DUNGEON_LIMITS` + `canStartPartyDungeon` matrix.
- API 25 integration test (`party-dungeon.service.test.ts`, real Postgres) — createRoom matrix (leader / not-leader / not-in-party / invalid-dungeon / duplicate-active-room), join (party-member / non-member / no-character), ready/unready (room not lobby), startRun gate matrix + auto-resolve, reward claim CAS (success / double / not-found / not-completed), cancelRoom (leader-only / not-lobby).
- FE 5 test (`PartyDungeonPanel.test.ts`) bao gồm empty + create flow, member join, leader start disable khi NOT_ENOUGH_MEMBERS, cancel confirm via ConfirmModal teleport-to-body, COMPLETED + PENDING reward claim.

#### Docs — Phase 20.1

- `docs/API.md` § Party Dungeon — `PartyDungeonController` (Phase 20.1) + WS event table.
- `docs/SECURITY.md` § 20.1 Party Dungeon — invariants, rate-limit, threat model.
- `docs/CHANGELOG.md` § Phase 20.1 (this entry).
