...
- **Phase 12 Story Dialogue Foundation** — CLOSED ✅ (PR #464): branching NPC dialogue catalog `STORY_DIALOGUES` (`DialogueNodeDef`/`DialogueChoiceDef` với conditions `quest_status`/`flag`/`seen_node`/`realm_min` + effects `mark_seen`/`set_flag`/`give_reward`/`advance_quest_step`); Prisma migration thêm `Character.storyDialogueSeen` + `storyFlags`; API `GET/POST /story/dialogue/:npcKey` atomic transaction; FE `StoryDialogueModal.vue` + Pinia store + axios client + i18n vi/en parity. Tests +33 (shared +5 / api +16 / web +12).
