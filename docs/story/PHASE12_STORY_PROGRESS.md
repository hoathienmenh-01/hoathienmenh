...
**Catalog foundation + quest runtime persistence + quest claim reward + NPC dialogue UI + Story Foundation Extension (Kim Đan + Nguyên Anh catalog) + Story Runtime MVP (Quest UI list + accept/claim) + Story PR-5 Main storyline Chapter 1 playable + Story PR-6 Combat kill hook → quest auto-track + Story Foundation Late-game wire + Phase 12.2.B DungeonTemplate + DungeonRun runtime + Phase 12.3 Inventory grant wire + Phase 12.4 per-monster `MonsterDef.lootTable` polish + Story Foundation Late-game encounter wire + Story discoverability QuestView dungeon hint + Phase 12.5 late-game story dungeon monster balance tuning + Phase 12 Story Dialogue Foundation (PR #464) + Phase 12.8.A/B/C/D/E Story Dungeon Instance Suite + Smoke Coverage (PR #465–#469) + Phase 12.9 Story Dialogue Branch Advanced + Phase 12.10.A NPC Affinity Foundation DONE (this PR).**

Hiện tại Phase 12 đã có:
- **Phase 12.10.D NPC Relationship Quest Chain** — CLOSED ✅ (this PR): tuyến nhiệm vụ duyên phận theo affinity tier — quest unlock dần, completing chain grants reward (affinity/LT/TN/EXP/items) + story flag. Shared `packages/shared/src/npc-relationship-quest-chains.ts` mới: `NpcRelationshipQuestChainDef` (`chainKey`/`npcKey`/`requiredAffinityTier`/`questKeys[]`/`dialogueNodeKeys[]`/`rewardHint`/`endingFlags`/`hidden?`/`title(En)`/`description(En)`); 3 chain mẫu khác arc: `relchain_moc_thanh_y_sect_path` (NPC tông môn, `quen_biet`, 3 quest realm-gated), `relchain_han_da_truce` (rival NPC, `quen_biet`, 1 quest có dialogue choice gate), `relchain_to_nguyet_ly_lineage` (`ban_huu`, `hidden=true`, 1 quest bloodline secret). Helper `npcRelationshipChainByKey`, `npcRelationshipChainsForNpc`, `chainClaimedFlagKey`. Validator `validateNpcRelationshipChainsCatalog()` invariants (refs `NPC_AFFINITY` / `QUESTS` / `STORY_DIALOGUE_NODES` / `AFFINITY_TIERS`; reward cap affinity≤40, linhThach≤500, tienNgoc≤10, exp≤600, items≤3 entry × ≤5 qty; unique `chainKey`; no duplicate questKey cross chain). API `apps/api/src/modules/npc-affinity/npc-relationship-chain.service.ts` mới: `listForCharacter` derive view 100% từ `CharacterNpcAffinity` + `QuestProgress` + `Character.storyFlags` (no DB entity, hidden chain `visible=false` khi tier chưa đủ); `claimChain(input)` atomic transaction: tier gate → all quests CLAIMED gate → JSON-path CAS guard via raw SQL `("storyFlags" ->> $flagKey) IS DISTINCT FROM '1'` (race-safe; 2 concurrent claim đúng 1 winner) → grant reward inside same tx (`NpcAffinityService.addAffinityTx` + `CurrencyService.applyTx` + `InventoryService.grantTx`, ledger reason `NPC_RELATIONSHIP_CHAIN_REWARD` refType `NpcRelationshipChain` refId `${chainKey}`) → ghi merged flags (claim + ending) vào `storyFlags`. Errors: `CHAIN_UNKNOWN` 404, `CHAIN_LOCKED_TIER` 403, `CHAIN_NOT_COMPLETABLE` 409, `CHAIN_ALREADY_CLAIMED` 409, `CHAIN_NPC_MISMATCH` 400. 2 endpoint mới: `GET /story/npc-affinity/:npcKey/quest-chain` + `POST /story/npc-affinity/:npcKey/quest-chain/:chainKey/claim`. `CurrencyLedgerReason` + `ItemLedgerReason` thêm `'NPC_RELATIONSHIP_CHAIN_REWARD'`. FE: `NpcAffinityPanel.vue` thêm tab "Tuyến nhiệm vụ duyên phận" collapse-by-default — list chain với progress bar (claimed/total), per-quest status (LOCKED/AVAILABLE/ACCEPTED/COMPLETED/CLAIMED) + colors, `RouterLink` CTA tới `/quests?focus=<questKey>` cho quest unlocked, reward preview block (affinity/LT/TN/EXP/items), locked-tier hint, claim button stateful (notUnlocked/notCompletable/claimed/loading); hidden chain (`visible=false`) ẩn hoàn toàn khỏi list. `QuestView.vue` render badge "Duyên phận" rose-200 cho quest thuộc catalog chain. Pinia `npcAffinity` store thêm action `loadChains(npcKey)` + `claimChain(npcKey, chainKey)` cập nhật in-place sau claim, sync affinity score. i18n keys mới `npcAffinity.chains.*` + `npcAffinity.chainErrors.*` vi/en parity. Tests: shared 12 test catalog invariant + helper PASS; api `npc-relationship-chain.service.test.ts` 11 PASS (list 4 + claim success + tier gate + completable check + unknown + idempotent retry + parallel race-safe Promise.allSettled exactly-1-winner + catalog query); web `NpcAffinityPanel.test.ts` thêm 12 test PASS (toggle + load + locked + progress + reward + RouterLink CTA + claim enable/disable + claimed state + error inline + hidden filter + empty), `QuestView.test.ts` thêm 1 test PASS (chain tag render). Idempotent guarantee: retry claim sau success luôn 409 `CHAIN_ALREADY_CLAIMED`, KHÔNG double-grant. Reward cap kỷ luật chain là content-gate, không phải power-source (xem `docs/BALANCE_MODEL.md` §2.9.7).
...
- **Phase 12.10.C NPC Shop + Hidden Unlocks** — CLOSED ✅ (this PR): biến NPC Affinity tier thành cổng mở khoá nội dung. Shared `packages/shared/src/npc-affinity-shop.ts` + `npc-hidden-unlocks.ts` mới: `NpcAffinityShopItemDef` (`npcKey`/`itemKey`/`requiredAffinityTier`/`cost`/`currency` LINH_THACH/TIEN_NGOC/`stockType` unlimited|daily|weekly/`dailyLimit?`/`weeklyLimit?`/`unlockHint(En)`); catalog 15 entry / 5 NPC chính (Lăng Vân Sinh / Mộc Thanh Y / Hàn Dạ / Tô Nguyệt Ly / Huyết La Sát). Helpers `npcShopForAffinity(npcKey, tier)` (filter unlocked tier-aware), `npcAffinityShopItem`, `toNpcAffinityShopItemView`. Validator `validateNpcAffinityShopCatalog()` invariants (item ref tồn tại trong `ITEMS`, NPC ref trong `NPC_AFFINITY`, tier ref trong `AFFINITY_TIERS`, cost positive int, stockType ↔ daily/weekly limit consistent, per-NPC daily-limit sum ≤30 anti-grind cap, tier cost cap xa_la/quen_biet ≤250 LT, ban_huu ≤1500 LT, tri_giao/tri_ky ≤2500 LT). `NpcHiddenDialogueUnlockDef`/`NpcHiddenQuestUnlockDef` (`npcKey`/`refKey`/`requiredAffinityTier`/`unlockReason(En)`) + helper `npcHiddenUnlocksForAffinity(npcKey, tier)` trả combined dialogue+quest list, marked `unlocked` flag, sort locked-first theo tier order. API `apps/api/src/modules/npc-affinity/npc-affinity-shop.service.ts` mới: `listShop(characterId, npcKey)` trả entries với `currentTier`/`unlocked`/`purchased`/`remaining`/`limitReached` (window count từ `ItemLedger.reason='NPC_SHOP_BUY'` + `refId='${npcKey}:${itemKey}'`); `buy(characterId, npcKey, itemKey, qty)` atomic Prisma `$transaction`: re-check tier, count purchased trong UTC daily/weekly bucket, verify limit, spend currency qua `CurrencyService.applyTx` negative delta, grant qua `InventoryService.grantTx`, ledger row `reason='NPC_SHOP_BUY'` + `refType='NpcAffinityShop'` + `refId='${npcKey}:${itemKey}'`; `listUnlocks(characterId, npcKey)` trả combined hidden unlocks. Endpoints mới: `GET /story/npc-affinity/:npcKey/shop`, `POST /story/npc-affinity/:npcKey/shop/buy`, `GET /story/npc-affinity/:npcKey/unlocks`. Error codes: 403 `INSUFFICIENT_AFFINITY_TIER`, 400 `INSUFFICIENT_FUNDS`, 429 `DAILY_LIMIT_REACHED`/`WEEKLY_LIMIT_REACHED`, 404 `ITEM_NOT_IN_SHOP`/`NPC_AFFINITY_UNKNOWN`, 400 `INVALID_QTY`/`NON_STACKABLE_QTY_GT_1`. `CurrencyLedgerReason` + `ItemLedgerReason` thêm `'NPC_SHOP_BUY'`. FE: `NpcAffinityPanel.vue` mỗi NPC card thêm toggle "Cửa hàng & ưu đãi đặc biệt" (collapsed default, lazy load shop+unlocks khi mở), entry name + cost + currency, locked badge với required tier label cho item chưa unlock, daily/weekly stock indicator (`Hôm nay 1/5`, `Tuần này 0/1`), buy button disabled khi locked/limit/loading; hidden unlocks list dưới shop (dialogue/quest entry + tier label + reason + ✓ marker khi unlocked); error states cho shop load + buy (insufficient tier/funds/limit). i18n keys mới `npcAffinity.shop.*`, `npcAffinity.shopErrors.*`, `npcAffinity.unlocks.*` vi/en parity. Tests: shared `npc-affinity-shop.test.ts` + `npc-hidden-unlocks.test.ts` (catalog invariant + i18n non-empty + tier monotonic); api `npc-affinity-shop.service.test.ts` 17 PASS (list shop tier filter, buy success path currency+inventory+ledger, INSUFFICIENT_AFFINITY_TIER, INSUFFICIENT_FUNDS, DAILY/WEEKLY_LIMIT_REACHED, item grant once, ITEM_NOT_IN_SHOP, NPC_AFFINITY_UNKNOWN precedence, listUnlocks tier-aware, startOfLocalWeek Monday helper); web `NpcAffinityPanel.test.ts` 7 test mới PASS (toggle collapse default, click toggle lazy loads shop+unlocks, locked items dimmed + disabled buy, click buy on unlocked calls API, buy error state, hidden unlocks list, daily/weekly stock indicators). **Anti-feature-creep**: KHÔNG full quest chain dài, KHÔNG romance/marriage, KHÔNG gacha, KHÔNG bán item phá economy (rare consumable + skillbook elemental basic + cosmetic-flavor only).
- **Phase 12.10.A NPC Affinity & Relationship Foundation** — CLOSED ✅ (this PR): mở foundation hệ quan hệ NPC. Shared `packages/shared/src/npc-affinity.ts` mới: `AffinityTierDef` 5 tier universal `xa_la(-1000) → quen_biet(10) → ban_huu(30) → tri_giao(60) → tri_ky(100)`; `NpcAffinityDef` per-NPC (`initialScore`/`minScore`/`maxScore`/`unlockHints[]`); catalog 5 NPC trụ cột (Lăng Vân Sinh / Mộc Thanh Y / Hàn Dạ / Tô Nguyệt Ly / Huyết La Sát) với cap khác nhau theo bản chất; helpers `affinityTierForScore`/`nextAffinityTierForScore`/`clampAffinityScore`/`validateNpcAffinityCatalog`. Hard cap chống farm: `AFFINITY_DELTA_CAP_PER_CHOICE=20` + `AFFINITY_DELTA_CAP_PER_QUEST_REWARD=40`. Story dialogue effect mới `change_affinity { npcKey, delta }` + condition mới `affinity_min { npcKey, score }` cùng validator catalog. Catalog Hàn Dạ +2 node minh hoạ (`story_dlg_han_da_friendly_chat` choice +10 warm / -5 cold + `story_dlg_han_da_inner_secret` gate `affinity_min:han_da≥30`). Prisma migration `20260606000000_phase_12_10_a_character_npc_affinity` thêm 1 table additive `CharacterNpcAffinity` (`id`/`characterId` FK cascade/`npcKey`/`score` Int + UNIQUE `(characterId, npcKey)` + 2 index). API `NpcAffinityService.addAffinityTx` atomic upsert + clamp + tier diff; `listForCharacter`/`getForNpc`/`loadScoreMap` read view lazy fallback `initialScore`. 2 endpoint mới `GET /story/npc-affinity` (list all NPC + caps) + `GET /story/npc-affinity/:npcKey` (single view với `currentTier`/`nextTier` + `pointsToReach` + `unlocks[]` reached flag). Story dialogue integration: `evaluateCondition` handle `affinity_min` (lookup `CharCtx.affinityByNpc`); `applyChoice` apply `change_affinity` atomic cùng `$transaction` với `give_reward`/`set_flag`/`mark_seen`; pre-flight cap check; `change_affinity` kê vào `hasGrantEffect` nhánh — retry choice `seen.includes(node.id)` → `ALREADY_APPLIED` no double-grant; `loadCtx` load score map; response `StoryDialogueChoiceResult.affinityChanges[]` cho FE toast/tier-up badge. FE: `apps/web/src/components/NpcAffinityPanel.vue` mới (5 NPC list + score progress bar + tier label + next-tier hint + unlock list); `apps/web/src/api/npcAffinity.ts` + `apps/web/src/stores/npcAffinity.ts`; `NpcView.vue` mount panel + auto-refresh sau dialogue; `StoryDialogueModal.vue` show toast `+N thân thiện` + tier-up. i18n vi/en parity. Tests +43 (shared +18+5 / api +15+8 / web +12). **Anti-feature-creep**: KHÔNG NPC gift, KHÔNG NPC shop, KHÔNG quest reward `change_affinity` (defer 12.10.B), KHÔNG affinity ledger granular, KHÔNG admin grant endpoint.
- **Phase 12.9 Story Dialogue Branch Advanced** — CLOSED ✅ (this PR): nâng cấp Phase 12 Story Dialogue Foundation thành multi-step branching tree với NPC nhớ lựa chọn cũ. Shared `packages/shared/src/story-dialogues.ts`: thêm condition `choice_made { nodeId, choiceKey }` (specificity score 5 same band quest_status — server pick followup node trước fallback) + effect `clear_flag { flagKey }` (xoá entry khỏi storyFlags, no-op nếu chưa set; cho plot reversal/apology arc); catalog +3 Hàn Dạ nodes minh hoạ multi-step tree (`story_dlg_han_da_first_meet` set `han_da_relation` rival/neutral; `story_dlg_han_da_followup_rival` gate `choice_made(first_meet, rival)` follow-up spar offer; `story_dlg_han_da_followup_neutral` gate `choice_made(first_meet, neutral)` follow-up neutral path; `story_dlg_han_da_resolution_apology` gate `seen(followup_rival)` + `flag_equals(rival)` choice apologize carries `clear_flag(han_da_relation)` revert sang neutral KHÔNG đụng `han_da_spar_arranged`). Validate two-pass build choiceMap nodeId → set choiceKey rồi verify `choice_made` ref. `storyDialogueAllFlagKeys()` cover `clear_flag`. Prisma migration `20260604000000_phase_12_9_story_dialogue_branch_advanced` thêm `Character.storyDialogueChoices Json @default('{}')` (nodeId → choiceKey, last-write-wins per node). API `StoryDialogueService` extend: `CharCtx.choices` + `loadCtx` đọc `storyDialogueChoices` + `evaluateCondition` handle `choice_made` + `summarizeConditionForReason` thêm `choice_made:nodeId=key` debug + `applyChoice` ghi `choices[node.id] = choice.key` last-write-wins + handle `clear_flag` effect (delete from flags map) + persist cùng `storyDialogueSeen`/`storyFlags`. Response shape: `StoryDialogueChoiceView.previouslyChosen: boolean` (player từng pick exact choice — KHÔNG disable, chỉ hint), `StoryDialogueNodeView.previousChoiceKey: string | null` (last-pick at this node), `StoryDialogueChoiceResult.choices: Readonly<Record<string,string>>` (snapshot post-apply để FE store sync). FE: `apps/web/src/api/storyDialogue.ts` types extended; `StoryDialogueModal.vue` v-else-if branch thêm "Đã chọn lần trước" badge khi `previouslyChosen=true && available && !alreadyApplied` với `data-testid="story-dialogue-last-{key}"` + amber-300 hint (alreadyApplied có precedence, không double-render). i18n vi/en parity: `storyDialogue.lastChosen`. Tests +14 (shared +5 specificity rank + multi-step + flag coverage + clear_flag invariant + validateConditionRef; api +7 choice_made gating + storyDialogueChoices persistence + clear_flag idempotent + previouslyChosen wiring + last-write-wins; web +2 badge render + alreadyApplied precedence). **Anti-feature-creep**: KHÔNG rewrite quest system, KHÔNG cutscene, KHÔNG dungeon runtime, KHÔNG Phase 13.2.
- **Phase 12.8.E Story Dungeon Smoke Coverage** — CLOSED ✅ (PR #469): test-only PR thêm `scripts/smoke-story-dungeon.mjs` (35 step Node smoke chạy real HTTP vs API local) cover full player flow — auth gates (5 endpoint × no-auth → 401), register + onboard fresh char, GET list pre-onboard 404 NO_CHARACTER, target locked → start 403 DUNGEON_LOCKED, drive `phamnhan_main_01` to COMPLETED via real /quests/accept + /quests/progress (talk×2) + admin /quests-track (kill son_thu ×3), accept `phamnhan_realm_01` + progress step_01 explore, target now status=available, start ACTIVE run idempotent, premature clear 409 RUN_STEP_INVALID, advance×3, advance out-of-range 409, clear → CLEARED, re-clear 409 RUN_NOT_ACTIVE, GET /quests/me verify quest progress no-regression, claim → granted 80 LT + 150 EXP + 1 linh_lo_dan, double-claim 409 RUN_ALREADY_CLAIMED, oneTime re-start 409 DUNGEON_ALREADY_CLEARED, register player2 → advance/claim non-owner 403 RUN_NOT_OWNED, player2 list → activeRun=null, target locked (data isolation). Wire `pnpm smoke:story-dungeon` trong root `package.json`. **Bugs found**: 0 (35/35 step PASS local). **Test-only — KHÔNG đụng runtime/component/i18n/migration.** Verification: api typecheck ✅ / api `--run story` 59 PASS / api `--run dungeon` 82 PASS / web typecheck ✅ / web `--run StoryDungeon` 74 PASS / web `--run Quest` 36 PASS / pnpm build ✅ / `pnpm smoke:story-dungeon` 35/35 PASS local.
- **Phase 12.8.D Story Dungeon UI Test Coverage** — CLOSED ✅ (PR #468): test-only PR fill §F web test coverage gap left after PR #467. 4 component test file mới: `StoryDungeonView.test.ts` (13 case render locked/available/cleared + start/claim/error fallback), `StoryDungeonRunPanel.test.ts` (23 case advance/clear/claim button state + emits + dialogue button), `StoryDungeonDialoguePanel.test.ts` (11 case open/close + render NPC dialogue), `StoryDungeonRewardModal.test.ts` (11 case render reward chips + close emits). Extend `QuestView.test.ts` +7 Story Dungeon CTA + `HomeView.test.ts` +8 Home CTA. **Test-only — KHÔNG đụng runtime/component code/i18n.** Verification: shared 1504 / web full **1372** PASS (+73 case) / api full 2188 / pnpm build ✅.
- **Phase 12.8.C Story Dungeon FE + Dialogue/Reward Polish** — CLOSED ✅ (PR #467): biến Story Dungeon từ backend runtime (12.8.B) thành playable FE experience. Web API client `apps/web/src/api/storyDungeon.ts` (6 axios function envelope-based) + Pinia store `apps/web/src/stores/storyDungeon.ts` (state + computed + actions `load/start/advance/clear/claim/findDungeon/findDungeonForQuest`). UI Components 4 file mới: `StoryDungeonView.vue` (main list view với header counter + filter + dungeon cards + active run panel inline + dialogue/reward modal); `StoryDungeonRunPanel.vue` (active run title + step progress + current monster + advance/clear/claim button); `StoryDungeonDialoguePanel.vue` (light Teleport modal + Esc/backdrop close — read-only NPC dialogue trước/sau dungeon); `StoryDungeonRewardModal.vue` (reward grant display sau claim). Integration: `QuestView.vue` CTA "Vào bí cảnh cốt truyện" + `HomeView.vue` CTA card khi available + `AppShell.vue` nav link `/story-dungeons` + router lazy-load route. Light backend polish: controller `activeRun?` enrich + service `findActiveRunForCharacter` helper — KHÔNG sửa runtime/migration. i18n vi/en parity 90+ key. Tests +16 store actions. Verification: web typecheck ✅ + `--run StoryDungeon` 16 PASS / `--run Quest` 29 PASS / web test full **1299** PASS ✅ / api typecheck ✅ + `--run story` 59 PASS ✅ / pnpm build ✅. **Phase 12 Story Dungeon Instance Suite COMPLETED** ✅ — Phase 12.8.A catalog + 12.8.B runtime + 12.8.C FE all CLOSED. **Next roadmap**: (1) Phase 12 Story Dialogue Branch advanced; (2) Phase 13.2 Cross-sect seasonal expansion; (3) CSP production verify; (4) Admin LiveOps advanced controls schedule editor.
- **Phase 12.8.B Story Dungeon Runtime + Quest Integration** — CLOSED ✅ (PR #466): biến Story Dungeon từ catalog read-only thành playable runtime. **Design**: Option B (new `StoryDungeonRun` model) thay vì reuse `DungeonRun` vì story dungeon có lifecycle khác hẳn farm dungeon (one-time + auto-advance quest step + reward differentiation `STORY_DUNGEON_REWARD`). Prisma migration `20260603000000_phase_12_8_b_story_dungeon_runtime`: enum `StoryDungeonRunStatus { ACTIVE, CLEARED, CLAIMED, FAILED }` + table `StoryDungeonRun` (`id`/`characterId` FK cascade/`templateKey`/`status`/`currentStep`/`killedMonsters` Json/`startedAt`/`clearedAt?`/`claimedAt?`/`updatedAt` + 3 index). 4 endpoint mutation `POST /story/dungeons/:key/start` (idempotent reuse ACTIVE run + reject `DUNGEON_LOCKED` + `DUNGEON_ALREADY_CLEARED`) + `POST /story/dungeons/:runId/advance` (ownership + `RUN_NOT_ACTIVE` + `RUN_STEP_INVALID`) + `POST /story/dungeons/:runId/clear` (CAS guard `updateMany status: ACTIVE → CLEARED` + clear hook `applyQuestStepAdvance` fail-soft) + `POST /story/dungeons/:runId/claim` (CAS guard `updateMany status: CLEARED → CLAIMED` race-safe + reward grant atomic SAU CAS qua `CurrencyService.applyTx` reason `STORY_DUNGEON_REWARD` + `InventoryService.grantTx` reason `STORY_DUNGEON_REWARD`). Quest integration fail-soft: clear → `QuestService.advanceStep` chỉ khi quest ACCEPTED + step chưa đủ; retry clear KHÔNG double progress. Anti-abuse: start idempotency / claim idempotency / concurrent claim 1-winner / no double reward / no double quest progress / locked reject / non-owner reject. Tests +15 (story-dungeon.service.test.ts) cover full 15 spec case (list / locked-cannot-start / start success+idempotent / one-time after CLAIMED / advance+invalid advance / clear+invalid clear / clear updates quest full+partial / claim success / double claim reject / concurrent claim 1-winner / non-owner reject / retry clear no-double-progress / fail-soft quest service throw). **Next**: Phase 12.8.C Story Dungeon FE + Dialogue/Reward Polish.
...
- **Phase 12.8.A Story Dungeon Catalog + API Foundation** — CLOSED ✅ (this PR): shared catalog `STORY_DUNGEONS` (`packages/shared/src/story-dungeons.ts`) — `StoryDungeonTemplateDef` (key + i18n title/desc + `requiredQuestKey` + `requiredQuestStep?` + `regionKey: RegionKey` + `recommendedRealm` + `minRealmKey?` + `monsterKeys[]` + `bossKey?` + `entryDialogueKey?` + `clearDialogueKey?` + `rewardHint?` + `oneTime` + `enabled`) + `StoryDungeonRewardHint` (linhThach / tienNgoc / exp / items[]). 4 catalog seed gắn 4 main quest đầu tiên: Phàm Nhân Hậu Sơn Linh Tuyền Động (`phamnhan_realm_01:step_01` — son_coc), Luyện Khí Hắc Lâm Tâm Thử (`luyenkhi_main_01:step_02` — hac_lam, minRealm=luyenkhi), Trúc Cơ Mộc Huyền Lâm Ký Ức Cổ Thụ (`truc_co_main_01:step_03` — moc_huyen_lam, minRealm=truc_co), Kim Đan Kim Sơn Thiên Lò Lệnh (`kim_dan_main_01:step_02` — kim_son_mach, minRealm=kim_dan). Reuse 14 monster + 4 region + 4 dialogue node hiện có → zero orphan. Helpers `storyDungeonByKey` / `storyDungeonsForQuest` / `computeStoryDungeonStatus` (returns `'locked'/'available'/'cleared'`) / `availableStoryDungeonsForQuestState` + invariant validator `validateStoryDungeonCatalog`. API `StoryDungeonModule`: 2 endpoint read-only `GET /story/dungeons` (list `enabled=true` + status compute) + `GET /story/dungeons/:key` (single template view). Response `StoryDungeonView` (key + i18n + region + realm + monsters[preview] + boss[preview] + rewardHint + status). KHÔNG Prisma migration, KHÔNG mutation, KHÔNG runtime advance/claim, KHÔNG FE. Tests +36: shared +25 (catalog integrity / helpers / `computeStoryDungeonStatus` x6 / `availableStoryDungeonsForQuestState` filter), api +11 (NO_CHARACTER / baseline locked / status x5 / view shape / getByKey / DUNGEON_NOT_FOUND / mutation reflect). **Next**: Phase 12.8.B Story Dungeon Runtime + Quest Integration (Prisma `StoryDungeonRun` + 4 endpoint mutation + reward atomic + auto-advance quest step + entry/clear dialogue trigger).
- **Phase 12 Story Dialogue Foundation** — CLOSED ✅ (PR #464): branching NPC dialogue catalog `STORY_DIALOGUES` (`DialogueNodeDef`/`DialogueChoiceDef` với conditions `quest_status`/`flag`/`seen_node`/`realm_min` + effects `mark_seen`/`set_flag`/`give_reward`/`advance_quest_step`); Prisma migration thêm `Character.storyDialogueSeen` + `storyFlags`; API `GET/POST /story/dialogue/:npcKey` atomic transaction; FE `StoryDialogueModal.vue` + Pinia store + axios client + i18n vi/en parity. Tests +33 (shared +5 / api +16 / web +12). Phase 12.9 nâng cấp foundation này — xem mục "Phase 12.9 Story Dialogue Branch Advanced" ở trên.
