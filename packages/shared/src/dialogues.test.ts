...
describe('DIALOGUES catalog integrity (Phase 12 PR-1)', () => {
  it('has unique ids', () => {
  ...
  it('dialoguesByNpc returns correct count', () => {
    expect(dialoguesByNpc('npc_lang_van_sinh').length).toBe(2); // default + truc_co branch
    expect(dialoguesByNpc('npc_moc_thanh_y').length).toBe(2); // default + luyen_khi branch
